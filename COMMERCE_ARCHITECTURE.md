...
One browser action now orchestrates multiple calls to the same protected route. The first phase verifies identities and paginates both summary lists; product phases read at most 12 details through one shared four-request pool; conditional file phases read at most 20 file records. Every GET has a bounded retry, leaving each invocation below 50 worst-case provider calls. The server signs the short-lived manifest and every normalized evidence chunk using existing server-only session protection, verifies completeness and signatures during final assembly, and rejects expired, altered, duplicate, missing, or cross-run evidence. Only verified final assembly writes the bounded completion audit. Credentials never enter a body or response, and provider evidence is not persisted.
