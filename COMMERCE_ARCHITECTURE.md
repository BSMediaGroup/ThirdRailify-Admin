...
`commerce_orders.customer_id` is nullable for legacy evidence and restrictive for linked history. Every new normal checkout persists Customer linkage in the same D1 batch as the order, but the encrypted order delivery snapshot also retains the checkout-time contact. Current Customer or Account changes therefore do not rewrite historical order truth. TEST and LIVE counts and integer CAD aggregates remain separate.

Migration `0019_account_address_book.sql` extends that authority with an encrypted current Customer phone and purpose-bound encrypted reusable delivery addresses. The address label, destination country, lifecycle, default flag, revision, and timestamps remain queryable; recipient, company, street, locality, region, postal code, and phone stay inside AES-256-GCM envelopes. One Customer can have at most ten active addresses and one default. Address replacement/default selection uses optimistic revision and ownership checks; current-address deletion is a deliberate hard delete and never cascades into or rewrites an immutable order delivery snapshot.

Public resolves its host-only Account session before signing a bounded HMAC request to `/api/account-commerce/internal/*`. Admin independently verifies the exact Public origin, method/path/body digest/timestamp, active Account authority, linked Customer, rate limit, and resource ownership. Safe projections expose current contact, reusable addresses, TEST/LIVE-separated order summaries, and owned order details without Customer IDs, Stripe identifiers, webhook data, provider credentials, encryption envelopes, or audit bodies. Public receives no Commerce D1 binding or encryption key.
...
Address forms use pinned offline `country-region-data@4.1.0` data in both independent applications. Native selectors show human names, while saved-address, shipping, checkout, and business-address contracts continue to carry ISO alpha-2 country codes and dataset subdivision short codes. Legacy display normalization is client-side and occurs only in form drafts; persisted data changes only on explicit save. Known codes are translated only for presentation, immutable order snapshots are untouched, and no geography API, schema migration, D1/R2 resource, or new consent category is involved.
