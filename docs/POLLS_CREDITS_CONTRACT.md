...
## Authorized manual vote increments

Migration 0043 adds the immutable `poll_manual_votes` ledger. Active Admins with Poll management authority and accounts with active Poll creator grants may add 1?10000 votes per request to an option in an open, publicly listed Poll. The Public gateway requires the existing signed-in session, origin check and CSRF token; the signed Admin endpoint rechecks the account/grant. Client-supplied account identities are not trusted. Request IDs deduplicate retries and reject different payloads; the ledger records actor, option, amount and time. A database trigger verifies Poll state and option membership at insertion. Manual totals are projected separately as `manualVotes`, included in totals/percentages, and leave ordinary voter identities and Rant/gift reconciliation unchanged.

Public detail and quick view offer a hover/focus-revealed amount stepper and Add action only after authenticated approval is resolved. Popouts and closed Polls have no increment action. Successful adds use ephemeral notifications. The new permission/replay/revocation/ordinary-vote regression passes, alongside the existing Poll suite (16/16). Migration 0043 was backed up and applied in production with no foreign-key errors and no live vote additions.

Release verification: Admin 42fed55e, Public 65dc477d. Production main and Poll JS/CSS match the tested build. Anonymous increment POST returns 401; both live Poll APIs return successful JSON with manual vote projections. Connected browser acceptance passes for Aboot Nothing +7, regular Poll detail +3 and quick view +2, logged-out absence, hover reveal and desktop/mobile control geometry. Screenshots inspected under .artifacts/poll-increments/both-types. All vote-writing acceptance used isolated D1/R2; no live votes were added.


## Upcoming galleries and reset history - 2026-09-08

Saved never-opened drafts are publicly listed as Upcoming for regular and Aboot Nothing Polls. Migration 0044 publishes existing never-opened drafts; archived Polls remain hidden. Upcoming galleries are separately paginated and searchable. Detail and quick-view voting buttons remain disabled until opening, and Admin/Public APIs still enforce open-state voting. Upcoming artwork is projected and served by the Media Worker. Closed Polls with an opening timestamp populate Past results.

Admin `Reset to Upcoming` retains a snapshot of previous public results, privately archives ordinary voter records, and starts current totals at zero without changing the Poll URL. Reset snapshots are visible under Previous results on the detail page. Manual and credit ledgers remain intact and are excluded from new-run totals through history mappings. Unallocated credits are discarded with an Admin audit record; previous provider windows cannot contribute late votes. The reset is atomic and guarded by lifecycle and result revisions. Confirmation explains zeroing and unallocated-credit disposal. Production reset was not exercised against user data.

Validation: Poll suite 17/17, additional paid-reset/late-window test 1/1, Media Worker tests 4/4, builds/typechecks and scoped lint pass. Connected browser acceptance covers both Upcoming galleries, both Admin reset buttons, disabled quick-view voting, saved history totals and opening into enabled voting; captures were inspected. One browser-process closure was followed by a successful rerun. Local evidence: .artifacts/poll-upcoming/acceptance.

Production: Admin 9e8b09fa, Public fe8f183e, Media Worker 9060bb19-27a8-48ca-bc93-3398bd278e86. Migration 0044 applied after protected backup; no pending migrations or foreign-key violations. Read-only checks found two upcoming regular Polls and one Aboot Nothing matchup, with successful upcoming artwork responses. No production Polls were reset. Public live browser evidence is under its .artifacts/poll-upcoming directory.
