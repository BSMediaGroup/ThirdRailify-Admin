...
Final readback covers every runtime: Public production uses `https://thirdrailify.com`, `https://admin.thirdrailify.com`, `https://cdn.thirdrailify.com`, and `THIRDRAILIFY_DOMAIN_CUTOVER_ACTIVE=true`; Public preview retains the two Pages origins and has the cutover switch false. Admin production uses the same three custom authorities, has the cutover switch true, and has Google enabled; Admin preview retains Pages origins, has the switch false, and has Google disabled with no preview OAuth secrets. The Commerce Worker targets the custom Admin machine API and store `18668025`. The Media Worker is the active `cdn.thirdrailify.com` authority. The bot targets only the custom Admin ingest endpoints. Production `pages.dev` references are limited to redirect/machine compatibility and the explicitly retained Discord/GitHub OAuth callbacks.
...
| --- | --- | --- | --- |
| Discord OAuth | Client ID/encrypted secret configured; only pre-cutover successful sign-ins exist in the production audit | `https://admin.thirdrailify.com/api/auth/oauth/discord/callback` | Retain the Pages callback until one legitimate custom-domain sign-in passes |
| Google OAuth | Production gate enabled; scopes remain exactly `openid email profile`; legitimate Public and Admin custom-domain sign-ins created/reused the centralized Account identity and preserved server roles | `https://admin.thirdrailify.com/api/auth/oauth/google/callback` | Remove the legacy Pages callback now; preview Google is disabled and has no preview OAuth secrets |
| GitHub OAuth | Client ID/encrypted secret configured; production has no GitHub identity or successful sign-in audit | `https://admin.thirdrailify.com/api/auth/oauth/github/callback` | Retain the Pages callback until one legitimate custom-domain sign-in passes |
| X OAuth | Legitimate custom-domain sign-in passed after aligning the confidential token request with X's contract and rotating the matching OAuth 2.0 Client Secret; D1 recorded a normal-user identity and consumed Public handoff | `https://admin.thirdrailify.com/api/auth/oauth/twitter/callback` | Remove the legacy Pages callback now |
...
| Printful | Store `18668025` and token scope were verified. The single POST and final GET both returned HTTP 200 JSON with top-level `data` and `extra`; the final GET shows the exact URL, 12-event set, public key, and no expiry | `https://admin.thirdrailify.com/api/webhooks/printful` | Configuration/readback are verified, but the signing secret is not in verified custody and signed delivery remains unproven. Use the [support escalation](PRINTFUL_V2_WEBHOOK_SUPPORT.md); final totals are GET 3, POST 1 and no retry is authorized |
...
Permanent operator defaults in `commerce-launch.mjs`, Commerce media backfill/verification, and the retained Stripe acceptance verifier use the custom origins. The PayPal setup command derives the canonical Admin origin. Google and X have passed legitimate custom-domain acceptance, so their legacy Pages callbacks should be removed. Discord and GitHub retain the transition callback only until their own custom-domain acceptance passes. Old Admin machine endpoints remain compatible independently of this provider-specific callback disposition.

## Printful signed webhook

The canonical V2 receiver is `https://admin.thirdrailify.com/api/webhooks/printful`. The initial readback was empty. The bounded setup then sent the exact reducer-supported 12-event intersection with `default_url` set to the custom Admin receiver, `expires_at=null`, and store-scoped token handling for store `18668025`. Printful returned HTTP 200 JSON whose top-level keys were `data` and `extra`; `result` was absent, contrary to the current official response contract. The strict parser stopped, accepted no signing keys, and changed no secret binding.

The final authorized `GET /v2/webhooks?show_expired=true` returned the same `data`/`extra` top-level shape and proved delayed persistence: the default URL is exact, the event set contains all 12 requested reducer-supported events, a public key is present, and expiry is null. Totals are GET `3/3` and POST `1/1`; no further Printful call is authorized. Final states are `configured=yes`, `readback_verified=yes`, `signing_secret_in_verified_custody=no`, and `signed_delivery_evidence=no`. Existing encrypted secret names are stale presence-only evidence, not proof of the current public-key identity. The receiver remains fail-closed until Printful provides a supported key-recovery or authorized recreation path. See [PRINTFUL_V2_WEBHOOK_SUPPORT.md](PRINTFUL_V2_WEBHOOK_SUPPORT.md). Receiver fixtures prove raw-body HMAC-SHA256 with hex-secret decoding, public-key/store checks, rejection paths, idempotency, and unsupported-event rejection. Preserved draft order `174104132` remains TEST, draft, unconfirmed, and unfulfilled.
