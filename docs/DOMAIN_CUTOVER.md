...
| Printful | Store `18668025` and token scope were verified. The one authorized POST returned HTTP 200 JSON with top-level `data` and `extra`, while the official contract and strict parser require `result`; no usable creation keys were available | `https://admin.thirdrailify.com/api/webhooks/printful` | `PRINTFUL_PROVIDER_SUPPORT_REQUIRED`; GET count 2, POST count 1. No retry/readback was attempted after the contract mismatch |
...
The canonical V2 receiver is `https://admin.thirdrailify.com/api/webhooks/printful`. The first GET returned no configuration. The bounded setup then sent the exact reducer-supported 12-event intersection with `default_url` set to the custom Admin receiver, `expires_at=null`, and store-scoped token handling for store `18668025`. Printful returned HTTP 200 JSON whose top-level keys were `data` and `extra`; `result` was absent, contrary to the current official response contract. The strict parser therefore stopped with `PRINTFUL_PROVIDER_SUPPORT_REQUIRED`, no signing keys were accepted, and no secret binding was changed. Totals are GET `2/3` and POST `1/1`; the final GET was deliberately not consumed after the provider-contract failure. Final states are `configured=no`, `readback_verified=no`, and `signed_delivery_evidence=no`. Existing encrypted secret names are stale presence-only evidence, not proof of the current signing identity. Receiver fixtures prove raw-body HMAC-SHA256 with hex-secret decoding, public-key/store checks, rejection paths, idempotency, and unsupported-event rejection. Preserved draft order `174104132` remains draft/unconfirmed.
