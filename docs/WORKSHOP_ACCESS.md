...
Master sessions edit provider-profile restrictions in a native modal lightbox rather than expanding the account row. It groups Replicate, OpenAI and xAI separately from Pexels, Pixabay and Unsplash, shows enabled-profile and restriction totals, restores focus to its launcher on close, and retains the existing compare-and-swap revision and audit contract. Stock-provider identity uses the approved `assets/icons/pexels-0.svg`, `pixabay-0.svg`, and `unsplash-0.svg` assets; no raw credential is projected into Admin.
