...
async function callPublicWatch(env, body, fetchImpl) {
  ...
  if (!secret || !publicOrigin || !publicOrigin.startsWith("https://")) throw new AuthFailure(503, "watch_management_not_configured", "Watch archive management is not configured.");
  ...
  try {
    response = await fetchImpl(`${publicOrigin}${PUBLIC_PATH}`, {
      ...
      headers: {
        ...
        Origin: normalizeOrigin(env?.THIRDRAILIFY_ADMIN_ORIGIN) || "",
  ...
  } catch { throw new AuthFailure(503, "watch_management_unavailable", "The Watch archive service is temporarily unavailable."); }
  ...
  if (!response.ok || !payload?.ok) {
    const status = response.status === 404 ? 404 : response.status >= 400 && response.status < 500 ? 400 : 503;
...
export { callPublicWatch, validateAction };
