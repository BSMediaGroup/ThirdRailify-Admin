...
export async function runPermanentPrintfulMigrationStep(env, session, fetchImpl = fetch, runtime = {}) {
  ...
  if (payload.migration.status === "completed") {
    await writeCommerceAudit(env, {
      actorAccountId: session?.accountId,
...
async function runProductStep(db, env, job, leaseToken, fetchImpl, now) {
  ...
  if (!product) {
    ...
    if (!product) {
      await updateJob(db, leaseToken, { status: "completed", phase: "completed", current_product_id: null, completed_at: iso(now()), updated_at: iso(now()) });
      return;
    }
...
async function createTargetProduct(db, env, job, product, leaseToken, fetchImpl, now) {
  ...
  const response = await providerRequest(db, env, job, leaseToken, { method: "POST", url: PRINTFUL_MIGRATION_ENDPOINTS.targetProducts, credential: targetCredential(env), operation: "target_create", allowedStatuses: [200], body: payload }, fetchImpl, now);
...
export function buildTargetCreatePayload(product, variants) {
  ...
  return {
    ...
    sync_variants: variants.map((variant) => ({
      ...
      files: variant.files.map((file) => ({ type: file.type, url: file.url, filename: file.filename, ...(file.options.length ? { options: file.options } : {}) })),
...
export function assertNoSourceFileIds(payload) {
  for (const variant of payload?.sync_variants || []) {
    for (const file of variant.files || []) {
      if (Object.hasOwn(file, "id") || !safeHttps(file.url) || file.type === "preview") throw migrationError("printful_cross_store_file_payload_invalid", "Target payloads must contain only validated HTTPS source artwork URLs and non-preview placements.");
    }
  }
  return true;
}
...
export function validateTargetProduct(result, product, expected) {
  ...
  for (const actual of result.syncVariants) {
    ...
    if (!sameMultiset(actualFiles.map((file) => `${file.type}:${file.filename}`), expectedVariant.files.map((file) => `${file.type}:${file.filename}`))) throw migrationError("printful_target_file_placement_conflict", `Target variant ${externalId} does not match the required print placements and files.`);
  }
...
async function expectedTargetVariants(db, productId) {
  ...
  const mappings = await db.prepare("SELECT source_file_id, source_url, filename FROM commerce_printful_file_mappings WHERE source_store_id = ?")
  ...
  return active.map((row) => {
    ...
    const files = plannedFiles.map((planned) => {
      ...
      return { type: cleanText(planned.type, 120), url: resolved.source_url, filename: safeFilename(resolved.filename || planned.filename), options: cleanOptions(planned.options) };
    });
    ...
    return { ...row, files, targetOptions: cleanOptions(safeJson(row.safe_metadata_json, {}).targetOptions) };
  });
}
