...
export async function businessInformationPayload(env, session) {
  ...
  const disclosureItems = [
    ...
    readinessItem("transaction_supplier_name", "Supplier legal identity", transactionDisclosure.facts.supplierName, transactionDisclosure.facts.supplierName ? profile.attestation.ownerConfirmed ? "OPERATOR ATTESTED" : "CONFIGURED - owner confirmation is included at launch" : "Required transaction-disclosure fact is absent"),
    readinessItem("transaction_supplier_phone", "Supplier telephone", transactionDisclosure.facts.supplierPhone, transactionDisclosure.facts.supplierPhone ? profile.attestation.ownerConfirmed ? "OPERATOR ATTESTED" : "CONFIGURED - owner confirmation is included at launch" : "Required transaction-disclosure fact is absent"),
    readinessItem("transaction_business_address", "Business-premises address", transactionDisclosure.facts.businessPremisesAddress, transactionDisclosure.facts.businessPremisesAddress ? profile.attestation.ownerConfirmed ? "OPERATOR ATTESTED" : "CONFIGURED - encrypted operator content" : "Required transaction-disclosure components are absent"),
...
export async function paymentsControlPlanePayload(env, session) {
  ...
  return {
    ...
    overall: {
      ...
      productionPayments: paypalProductionActive ? "configured" : "disabled",
...
function paypalProjection(row, authority = null) { const metadata = json(row?.safe_metadata_json, {}); const sandbox=authority?.credentials?.sandbox||{}; const live=authority?.credentials?.live||{}; const storeCaptureEnabled=authority?.settings?.paypal_live_capture_enabled===true; const donationCaptureEnabled=authority?.settings?.paypal_donation_live_capture_enabled===true; return { provider: "paypal", state: ["disabled", "deferred", "setup_required","connected"].includes(row?.status) ? row.status : "setup_required", integrationMode: row?.integration_mode === "direct_merchant" ? "direct_merchant" : "unavailable", environment: cleanText(row?.environment, 20) || "live", countryCode: cleanCode(row?.country_code, 2), currencyCode: cleanCode(row?.currency_code, 3), credentialConfigured: Boolean(live.clientIdConfigured&&live.clientSecretConfigured), sandbox, live, preferred:true, donationsEnabled: authority?.settings?.paypal_donations_enabled===true, membershipEnabled: false, shopCheckoutEnabled: authority?.settings?.paypal_store_checkout_enabled===true, captureEnabled:storeCaptureEnabled, storeCaptureEnabled, donationCaptureEnabled, webhookEvents:authority?.subscribedEvents||[],webhookUrl:authority?.webhookUrl||null,setupCommand:authority?.setupCommand||"npm run commerce:paypal -- status",latestVerifiedDelivery:authority?.webhooks?.[0]||null, attempts:authority?.attempts||[],donations:authority?.donations||[],diagnostics:authority?.diagnostics||[], providerMutationAvailable: false, lastVerifiedAt: cleanText(row?.last_synchronized_at, 80) || null, metadata }; }
