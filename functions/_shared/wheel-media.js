...
function projectAsset(row, env, isPublic = false) { const path = `/wheel-media/${encodeURIComponent(row.id)}`; return { id: row.id, purpose: row.purpose, url: isPublic ? publicMediaUrl(env, path) || `/api/wheels/media/${encodeURIComponent(row.id)}` : `/api/wheels/media/${encodeURIComponent(row.id)}`, contentType: row.content_type, byteSize: Number(row.byte_size), width: row.width == null ? null : Number(row.width), height: row.height == null ? null : Number(row.height), sha256: row.sha256, fileName: row.original_filename || undefined, animationCapable: row.content_type === "image/gif", createdAt: row.created_at }; }
