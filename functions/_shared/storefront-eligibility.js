...
export function storefrontEligibility(product, variants, storeId) {
  ...
  if (!product.target_printful_product_id) productReasons.push("product_mapping_missing");
  ...
  const rows = variants.map((variant) => {
    ...
    if (variant.currency_code !== "CAD" || !Number.isSafeInteger(variant.unit_amount) || variant.unit_amount <= 0) reasons.push("invalid_cad_price");
    if (!variant.target_printful_sync_variant_id || !variant.target_catalogue_variant_id || variant.target_printful_product_id !== product.target_printful_product_id || variant.fulfillment_mapping_status !== "mapped") reasons.push("mapping_invalid");
    const localReasons = [];
