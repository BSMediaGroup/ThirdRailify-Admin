...
if (!new Set(["plan", "catalogue-apply", "activate", "pause"]).has(command)) fail("Usage: npm run commerce:launch -- <plan|catalogue-apply|activate|pause> [--execute]");
if (!cookie) fail("COMMERCE_ADMIN_SESSION_COOKIE is required; this CLI never bypasses Admin authentication.");

const current = await request("/api/admin/commerce/launch");
if (command === "plan" || !execute) {
  process.stdout.write(`${JSON.stringify({ dryRun: command !== "plan", requestedCommand: command, ...current }, null, 2)}\n`);
  if (command !== "plan") process.stdout.write("No mutation was made. Add --execute with the protected CSRF environment value after reviewing this plan.\n");
...
const path = command === "catalogue-apply" ? "/api/admin/commerce/launch/catalogue-apply" : `/api/admin/commerce/launch/${command}`;
