...
export async function acceptance({browser,page,directory}) {
  ...
  try {
    ...
    await selects()[0].selectOption({label:names[0]});await selects()[1].selectOption({label:names[1]});
    await page.locator('.bracket-round').first().locator('.bracket-match').nth(1).click();await selects()[0].selectOption({label:names[2]});await selects()[1].selectOption({label:names[3]});
    await page.getByRole('button',{name:'Save draft',exact:true}).click();await waitSaved();await page.reload();await waitSaved();await capture('live-saved-draft');record.completed.push('create arrange save hard-reload');await save();
    await page.locator('.bracket-round').first().locator('.bracket-match').nth(1).click();await page.getByRole('button',{name:'Record manual result / bye',exact:true}).click();await page.getByLabel('Winner',{exact:true}).selectOption({label:names[2]});await page.getByLabel(`Score: ${names[2]} (blank = unknown)`,{exact:true}).fill('2');await page.getByLabel(`Score: ${names[3]} (blank = unknown)`,{exact:true}).fill('1');await page.getByLabel('Reason',{exact:true}).fill('Disposable acceptance historical record; not a real season result.');await action('Confirm labelled decision & advance');
    await page.locator('.bracket-round').first().locator('.bracket-match').nth(0).click();await page.getByRole('button',{name:'Create Poll',exact:true}).click();await page.getByLabel('Poll title',{exact:true}).fill(`${title} Poll A`);await capture('live-create-poll');await action('Create and link private Poll');
    const editHref=await page.getByRole('link',{name:'Edit Poll',exact:true}).getAttribute('href'),slug=new URL(editHref,ADMIN).searchParams.get('edit');record.polls.push({slug,title:`${title} Poll A`});await save();await capture('live-linked-poll');
    ...
    await page.goto(record.bracketUrl);await waitSaved();await page.locator('.bracket-round').first().locator('.bracket-match').nth(1).click();await page.getByRole('button',{name:'Link existing Poll',exact:true}).click();await page.getByLabel('Search all Aboot Polls',{exact:true}).fill(`${title} Poll B`);await page.getByRole('button',{name:'Map these opponents',exact:true}).click();await page.getByRole('button',{name:'Reverse option mapping',exact:true}).click();await capture('live-link-existing');await action('Confirm mapping & link');
    const secondHref=await page.getByRole('link',{name:'Edit Poll',exact:true}).getAttribute('href');record.polls.push({slug:new URL(secondHref,ADMIN).searchParams.get('edit'),title:`${title} Poll B`});await save();
    await page.getByRole('button',{name:'Review publication',exact:true}).click();await page.getByLabel('Public title',{exact:true}).fill(title);await page.getByLabel('Public URL slug',{exact:true}).fill(run);await page.getByLabel('Introduction',{exact:true}).fill('Temporary release acceptance roadmap. This is test data and will be unpublished after verification.');
    const png=await sharp({create:{width:1200,height:600,channels:4,background:'#26354c'}}).composite([{input:Buffer.from('<svg width="1200" height="600"><text x="70" y="260" fill="#efc65c" font-size="64">TEMPORARY ACCEPTANCE</text><text x="70" y="340" fill="white" font-size="34">Matchup Studio · disposable release verification</text></svg>')}]).png().toBuffer();
