...
- Removed the shared operational mailbox from tracked Wrangler `ADMIN_EMAIL_2` configuration so Stripe team-login context is not persisted by the application. The second environment-backed Master identity is now absent from tracked Wrangler vars and requires separate approved configuration if it remains operationally necessary.
