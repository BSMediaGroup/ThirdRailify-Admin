...
- Added a Gaming-specific compatibility preflight for every Gaming authority read/write. Missing or incomplete `0028_gaming_catalogue.sql` now returns `gaming_migration_required`, while configured-empty and query-failure states remain distinct; the Admin page renders a dedicated database-update panel and no fake summary values.
- Added authenticated `gaming.view`, 60/hour rate-limited in-dashboard Steam title search plus direct App ID/canonical URL resolution through a server-only adapter. The verified public Store `api/storesearch` and `api/appdetails` paths are explicitly best effort; normalized responses are capped/bounded, searches cache for 10 minutes, details for 12 hours, and 4.5-second timeout/provider failures never block manual editing or save.
- Added optional server-secret `STEAM_WEB_API_KEY` support for official `IStoreService/GetAppList` exact-App catalogue evidence. The browser receives no credential, raw provider payload, price data, arbitrary-fetch URL, cache key, or provider internals; exact Store-host/protocol/credential/port validation prevents SSRF-style URL abuse.
- Rebuilt the editor around a stable header, dedicated persistent-scrollbar body, and always-reachable footer. The scroll gutter is visible on overflowing desktop dialogs, wheel/touch scrolling is contained inside the editor, and geometry coverage proves the body reaches its true bottom without moving header/footer controls. Search results are responsive, keyboard-operable, bounded cards; selection prefills title, App ID/URL, actual provider artwork, description, genre, developer, and publisher without saving. Existing records fill blanks by default, expose explicit curated-text overwrite, and retain existing uploaded or remote artwork. External Search Steam and every manual field remain available.
- Added the public-safe `/api/gaming/rotation` projection with three-minute edge freshness and focused migration, schema, lifecycle, validation, capability, upload, provider fixture/cache/failure, public-order, and five-width browser coverage. One bounded read-only live acceptance verified Store search and App details; no provider mutation occurred. No deployment, remote migration/D1/R2 write, secret change, DNS action, or push was performed.
- Production activation: exported and hashed the live `thirdrailify-commerce` D1 backup, applied only `0028_gaming_catalogue.sql`, and verified the complete Gaming schema, six indexes, four seeded games/rotation rows, and an empty foreign-key check. Deployed the isolated Gaming/Steam Admin build from `eea4cba` as `c672a18c-9a66-49e2-adf6-a11bef654eec`; a route-manifest repair made the sanitized Admin projection and existing Public relay return the four seeded titles in order. Public was not redeployed, no Gaming record was created for acceptance, and the authenticated editor click-through remains intentionally unclaimed because the automation browser had no signed-in session.
- Authenticated production acceptance later proved repeated HTML `502` responses because both Steam Store and Steam Community reject Cloudflare Pages egress; a subsequent runtime SteamSpy fallback was also unsuitable. The final fail-closed design keeps those best-effort paths but ships a bounded build-time snapshot of 1,000 verified public App ID/title/developer/publisher/genre records, refreshed deliberately by `scripts/build-steam-fallback-catalogue.mjs`, so covered searches and App resolution require no external request at runtime. Search selection is applied before optional enrichment, preventing a second-request failure from erasing the App ID/title/URL. Isolated production deployment `ee0f55f0-53ae-49bf-bbfa-d7989351f85e` passed authenticated custom-domain acceptance: `WITCHER` search returned four results with HTTP 200, selecting App `292030` returned HTTP 200 and `Metadata loaded`, and direct `292030` lookup returned HTTP 200 from `embedded_steam_catalogue_fallback`. No game record, D1 row, R2 object, secret, DNS setting, or unrelated dirty commerce/PayPal file was changed by acceptance.
...
- Root cause: every Products-list star used the broad bulk mutation, rebuilt and serialized the full Commerce catalogue and all variants, then refreshed both the main and Featured product lists. One click therefore caused one full-catalogue POST plus two full-catalogue GETs; a page-wide busy flag locked every row, and repeated clicks repeatedly resolved Admin sessions against Account D1.
- The generic Admin Commerce limiter allowed only 30 mutations per hour. The 31st bulk Featured request returned HTTP 429, while non-JSON/global error fallback could surface the misleading `The account service is unavailable.` message. The recovered Public catalogue authorities remained populated; this hotfix does not change Public catalogue data or Public account limits.
- Added authenticated, CSRF-protected `POST /api/admin/commerce/products/:id/featured` under `commerce.catalogue.manage`. It accepts only an explicit boolean, performs an idempotent parameterized D1 update of `is_featured`, `featured_order`, and `updated_at`, writes Commerce audit evidence, returns only the authoritative affected product fields, and never invokes a provider or rewrites product price, visibility, fulfilment, or migration state.
- Product stars now update optimistically per row, reconcile the authoritative response without catalogue revalidation, roll back with a compact row error on failure, and synchronously lock only the same product while its write is pending. Different products remain concurrently interactive; search, filter, sort, page size, page, scroll, and catalogue state remain mounted.
- Introduced the bounded trusted-Admin `commerce_catalogue_mutation` bucket (240/hour, 15-minute block) for Featured, ordering, and bounded product bulk operations, keyed through the existing authenticated Admin limiter. Existing `account_commerce_read` (600/hour), `account_commerce_mutation` (120/hour), authentication, checkout, and provider limits are unchanged.
- Added authoritative API coverage plus realistic 50-product browser coverage for 20 rapid toggles, exact request counts, zero mutation-driven catalogue/auth/bootstrap refetches, same-product double-click locking, 409/429/500/network rollback, continued use of other rows, responsive layouts, and absence of account/catalogue failure banners or uncontrolled retries.
- No schema migration is required. Rollout requires only a reviewed Admin production deployment, followed by authenticated `/products` rapid-toggle/network acceptance and Public `/shop` catalogue read verification. No deployment, remote migration, D1/provider/payment/DNS/Cloudflare mutation, or Public repository change was performed locally.
...
- Redesigned Role Permissions & Scopes as a high-density policy console with an explicit keyboard-operable Master Admin / Full Admin / Regular User selector, unmistakable selected-role context, compact policy legend and search, subdued capability IDs, stronger domain hierarchy, and native accessible checkbox semantics presented as on/off switches. Master and Regular inspection states remain fixed, Full Admin restrictions remain denial-backed, and Full Admin viewers retain interactive inspection while every policy switch stays read-only.
- Refined Master edit mode so mutation actions appear only while a Master inspects Full Admin: the action bar reports exact dirty-change counts, Apply remains disabled when current, and reset retains the existing confirmation before deleting explicit denials to restore default parity. The prior overlay-prone sticky behavior is now an in-flow responsive action bar; layouts cover 1920, 1440, 1024, 768, and 390 px without horizontal overflow, control overlap, or covered policy content.
- Moved the existing `/access` Users / Access navigation entry directly between Watch / Broadcast and Audience Analytics without changing its route, icon, capability policy, or active-state behavior. Focused browser coverage now exercises all three inspection roles, native radio/switch keyboard behavior, locked and editable states, search, Apply/reset persistence, read-only Full Admin restrictions, responsive geometry, console cleanliness, and exact sidebar order.
...
## 2026-09-14 - Wheel entrant identity V2 and closed successors

- Added durable seven-character entrant codes, optional display suffixes, explicit provenance, exact recoverable source bindings, and immutable safe automation receipt detail while retaining the existing typed identity authority and all weights/media/appearance.
- Changed participant saves to revision-guarded row patches, added compact searchable/filterable participant management, Admin receipt-detail dialogs, and an isolated bounded/resumable 0050 release and backfill path that excludes pending Poll migration 0046.
- Added official winning-entry snapshots and idempotent Close & Create Next: the original becomes a stable Past Wheel; the private draft successor gets copied visual media/access and disabled automation definitions, with participants off by default and fresh IDs/codes when selected.
- Backed up production D1 to `C:\Users\TempAdmin\.codex\backups\thirdrailify-commerce-20260914-wheel-identity.sql` (7,356,154 bytes; SHA-256 `F46ECCF8A724B379ECAB0D6E79F6B6533F03AD57B80982BE46644E725835A638`), applied only reviewed migration `0050`, and backfilled all 284 existing entrants. Post-checks found 284/284 codes, no null/duplicate/invalid codes, unchanged 12 Wheels/2,814 total weight/64 receipts/0 official results/0 contributions, and clean foreign keys. Pending migration `0046` remains unapplied.
- Released Admin production deployment `b7a4b466-c51b-4351-91e7-ad0cb35ef970` from reviewed base `7535a8b`. Stable `admin.thirdrailify.com` serves the identical `index-vwqOXf3K.js` bytes (SHA-256 `CBA5268F65C8ED057820952CB88BC5ECE3BA8038DD7D3FB7425245C0CCE1B6FD`) as the immutable Pages URL; the stable Wheel projection and receipt route returned JSON with the expected cache policy and no sampled raw/internal leakage.
- Builds, lint, focused identity/automation/Stage/portability/renderer checks, responsive browser fixtures, and the representative 60-second spin profile passed. No Bot process or Bot state was changed. No production official spin, participant edit, close/successor creation, receipt inspection under an authenticated operator session, or other competition mutation was performed because no clearly labelled disposable private acceptance Wheel/authenticated session was available.
