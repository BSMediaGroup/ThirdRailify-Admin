-- Preserve existing assets while adding participant avatar uploads.
ALTER TABLE wheel_media_assets RENAME TO wheel_media_assets_before_avatars;
...
CREATE TABLE wheel_media_assets (
  ...
  purpose TEXT NOT NULL CHECK (purpose IN ('background', 'centre', 'segment_fill', 'avatar')),
...
SELECT id, wheel_id, purpose, object_key, sha256, content_type, byte_size, width, height,
       original_filename, lifecycle, uploaded_by_account_id, created_at, updated_at, deleted_at
FROM wheel_media_assets_before_avatars;

DROP TABLE wheel_media_assets_before_avatars;
...
CREATE UNIQUE INDEX wheel_segment_media_active_hash_idx
  ...
  WHERE lifecycle = 'active' AND purpose IN ('segment_fill', 'avatar');
