...
`/workshop/access` manages existing-account Lab approval, suspension, expiry and audit history through canonical Admin capabilities. Its premium black, charcoal, gold and muted-plum control surface keeps effective policy, explicit records and protected accounts legible from desktop through mobile; Overview remains the first workspace destination, while Workshop Access sits directly below Users / Access with a dedicated gateway icon. The Lab uses the existing login, Turnstile and target-bound handoff with host-only cookies. See [architecture, created files, migrations, backup and validation](docs/WORKSHOP_ACCESS.md). Private Lab conversations and media remain in dedicated Lab storage.

The Admin surface presents Workshop Access as a dedicated authority workspace: Overview remains the first sidebar destination, Workshop Access sits directly below Users / Access with its own workshop-gateway icon, and the responsive account directory distinguishes effective access, policy source, explicit records and protected accounts without changing the underlying permission contract. Master-only provider-profile policy editing opens in a native responsive lightbox, grouped into AI and Search & Stock providers; the supplied Pexels, Pixabay and Unsplash marks identify their credential policies without exposing secrets.

Master Admin can also inspect each account's effective Lab credential-profile access, restrict a provider to selected profiles, choose an account default, remove restrictions and review actor/target/provider before-and-after audit evidence. `workshop.profile_restrictions.manage` is a non-configurable Master-only capability; Full Admin and Regular User requests are rejected by the Function even if client controls are forged. Admin reads only safe Lab profile metadata through its `LAB_DB` binding; raw provider secrets never enter Admin. Additive Auth migration `migrations/0004_workshop_provider_profile_restrictions.sql` owns policies, grants and audit rows. Lab remains the source of profile metadata and enforces the policy again on every protected provider submission.

Profile-restriction file inventory: `migrations/0004_workshop_provider_profile_restrictions.sql`, the Workshop route in `functions/api/workshop/[[path]].js`, canonical capability/policy projections, `src/pages/WorkshopAccessPage.tsx`, `src/pages/workshop-access.css`, and focused authorization/browser coverage. No Commerce schema or site roles are changed.
...
The Admin exposes only sanitized, unauthenticated read projections under `/api/public/commerce/*`; the Public Pages project proxies them without owning a Commerce D1 binding. Production merchandise checkout, PayPal LIVE capture, and fulfillment submission are enabled through the canonical commerce launch authority, with the emergency pause remaining available as a separate fail-closed control. The one-time `stripe_test_checkout_enabled` acceptance harness remains closed after the successful first genuine Stripe TEST acceptance; it is preserved evidence, not a production dependency or operator-facing activation control.
...
Independent authenticated control room for Third Railify operations. The shared D1 account authority, real session/role enforcement, bounded account administration, and an Admin-only Canadian commerce control plane are implemented. PayPal is the preferred LIVE store and donation provider; Stripe is retained as configured but disabled/non-preferred. Payments status reads are read-only and do not perform provider or commerce-setting mutations.
...
- Public-origin `POST /api/commerce/shipping-quotes` and `POST /api/commerce/checkout` are the Admin-authoritative guest boundaries. They accept bounded local product/variant IDs, quantities, and normalized delivery input; bind an opaque rate quote to server-resolved cart and recipient fingerprints; encrypt the durable recipient snapshot; derive integer CAD subtotal/shipping/total; snapshot the order before PayPal; and enforce the active launch, payment, shipping, agreement, tax, catalogue, and emergency-pause authorities on every request.
- The historical capability-protected `POST /api/admin/commerce/test-checkout` acceptance harness remains closed. Its preserved TEST order and signed Stripe webhook evidence remain visible in `/orders` and `/commerce/payments`, but the harness is not a production gate and exposes no one-click reopen path.
...
- PayPal-first Canadian operating model using the Third Railify direct merchant account, server-created and server-captured Orders v2, Admin-only environment secrets, active store/donation authorities, `draft_then_confirm` Printful fulfillment, and Stripe retained as configured but disabled/non-preferred.
...
Account administration is operational in code. The separate commerce D1 is bound, private commerce configuration remains in Admin-only custody, and PayPal LIVE is the preferred store and donation provider. The preserved Stripe TEST acceptance can confirm only its already-linked historical TEST order after exact reference, Session, amount, currency, mode, payment-status, and environment checks; Stripe remains configured but disabled/non-preferred. Public checkout, PayPal LIVE capture, and Printful `draft_then_confirm` fulfillment are active through canonical launch authority, while live payout readiness remains independently unverified. The verified existing `thirdrailify-profile-media` bucket is declared locally through the Admin-only `THIRDRAILIFY_PROFILE_MEDIA` binding. Public receives no commerce binding or secret.
...
| `/commerce/payments` | Read-only PayPal-first Payments & Payouts control plane with canonical production activation, isolated preserved Stripe TEST evidence, separate LIVE/TEST summaries, webhook health, truthful payout boundaries, and configured-but-disabled Stripe posture |
| `/api/commerce/checkout` | Exact-Public-origin customer POST/OPTIONS endpoint; server-priced PayPal Orders v2 checkout behind canonical launch, catalogue, shipping, agreement, tax, and emergency-pause gates |
