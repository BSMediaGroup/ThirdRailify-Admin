...
- Featured-product changes require `commerce.catalogue.manage` and validate a bounded, duplicate-free list of existing displayable product IDs server-side before one D1 batch updates the full order. The public projection is GET-only and excludes titles, prices, images, metadata, accounts, permissions, and audit records.
