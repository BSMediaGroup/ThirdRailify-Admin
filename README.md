...
Account administration is operational in code. The separate commerce D1 is bound and migrated, its encryption secret and Stripe TEST credentials are held as Admin Cloudflare encrypted secrets, and the protected Stripe verification action performs only `GET /v1/account`, requires Canada and CAD, and stores safe proof in the provider row plus canonical setting. The deployed webhook receiver has accepted one real signed sandbox event into its idempotent receipt ledger and synchronizes the corresponding safe provider/setting proof. Checkout, live payments, live payout readiness, order mutation, and fulfillment remain disabled or unverified. The verified existing `thirdrailify-profile-media` bucket is declared locally through the Admin-only `THIRDRAILIFY_PROFILE_MEDIA` binding. Public receives no commerce binding or secret.
