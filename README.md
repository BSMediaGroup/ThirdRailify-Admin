...
The Admin exposes only sanitized, unauthenticated read projections under `/api/public/commerce/*`; the Public Pages project proxies them without owning a Commerce D1 binding. Normal checkout, live payment capture, and fulfillment remain globally disabled. A separate `stripe_test_checkout_enabled` pre-cutover gate permits only an authenticated Master Admin to generate the single Stripe-hosted TEST acceptance Session for the configured D1 product/variant. The permanent Printful migration is independently checkpointed and remains manually paused.

The acceptance candidate is local product `product-397267935`, slug `third-rail-farm-black-glossy-mug`, title **Third Rail Farm | Black Glossy Mug**, and local variant `variant-5019554081` (**11 oz / Black**) at CAD 15.00. The variant is target-verified/mapped and is the only temporary `is_sellable=1` acceptance variant; provider IDs remain Admin-internal. Signed Stripe webhooks remain payment authority, and even a paid TEST order keeps fulfillment `disabled / not_started` with no Printful order.
