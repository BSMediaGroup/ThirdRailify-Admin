...
export function RumbleSubscriberTrend({ source, snapshotId, total, asOf, refreshVersion }: { refreshVersion: number; source: string; snapshotId?: string; total?: number; asOf?: string }) {
  ...
  const toggle = (key: Series) => setVisible(previous => { const next = new Set(previous); if (next.has(key)) { if (next.size > 1) next.delete(key); } else next.add(key); return next; });
  return <section className="ri-trend" aria-label="Rumble subscriber trend" aria-busy={loading}>
    ...
    <div className="ri-trend__headline"><div><span>Total current subscribers</span><strong data-testid="subscriber-total">{total === undefined ? '—' : total.toLocaleString()}</strong></div><p>Distinct API-listed accounts<br /><span>{asOf ? `As of ${stamp(asOf)}` : 'Awaiting a qualified observation'}</span></p></div>
    <div className="ri-trend__legend" role="group" aria-label="Subscriber chart series">{series.map(s => <button key={s.key} aria-pressed={visible.has(s.key)} onClick={() => toggle(s.key)}><i style={{ background: s.color }} />{s.label}</button>)}</div>
