...
import { calculateAward, defaultAction, ruleFieldErrors, type ActionConfig } from '../lib/automation-model.mjs';

import { families, automationRequest, AutomationRequestError, type Rule, type Conditions, type WheelChoice, type Discovery, type TestResult } from '../lib/automation-client';

export function AutomationRuleEditor({ rule, wheels, discovery, csrf, busy, canManage, serverErrors, onChange, onSave, onClose }: {
  rule: Rule; wheels: WheelChoice[]; discovery?: Discovery; csrf: string; busy: boolean; canManage: boolean;
  ...
  return <form className="event-editor" aria-label="Automation rule editor" noValidate onSubmit={e => { e.preventDefault(); if (valid && !(textEvent && streamMode !== 'any' && !rule.conditions.livestreamId)) onSave(); }}>
    ...
    <div className="event-fields"><label>Event family<select aria-label="Event family" value={rule.eventType} onChange={e => { update({ eventType: e.target.value, conditions: {}, actionConfig: defaultAction(), duplicatePolicy: 'skip' }); setStreamMode('any'); }}>{families.map(([key, name]) => <option key={key} value={key}>{name}</option>)}</select>{hint('eventType')}</label></div>
    ...
    {stateOnly ? <div className="event-incompatible" role="status"><strong>Wheel entry action incompatible</strong><p>Livestream transitions have no actor. Add actor to Wheel is unavailable.</p></div> : <>
      ...
      <fieldset className="event-award"><legend>Entry award</legend><div className="event-fields">
        ...
        <label>{action.award.mode === 'per_gift' ? 'Entries per gift' : action.award.mode === 'per_amount' ? 'Entries per unit' : gift ? 'Entries per purchase' : rant ? 'Entries per Rant' : 'Entries per event'}<input aria-label={action.award.mode === 'per_gift' ? 'Entries per gift' : action.award.mode === 'per_amount' ? 'Entries per unit' : gift ? 'Entries per purchase' : rant ? 'Entries per Rant' : 'Entries per event'} type="number" min={1} max={100000} step={1} value={action.award.entriesPerUnit || ''} {...invalid('entriesPerUnit')} onChange={e => award({ entriesPerUnit: Number(e.target.value) })} />{hint('entriesPerUnit')}</label>
      ...
      <fieldset className="event-tester"><legend>Dry run — no action will be executed</legend><div className="event-fields">
        ...
        {textEvent ? <label>Sample message<input value={sample.text} onChange={e => { setSample({ ...sample, text: e.target.value }); setTest(null); }} /></label> : null}
      ...
      </div><div className="event-actions"><button type="button" className="secondary-button" onClick={() => { setSample({ actorLabel: 'ExampleUser', text: rule.conditions.exactText || 'ENTER', amountCents: 500, totalGifts: 5, giftType: rule.conditions.giftType || 'random', badge: rule.conditions.badge || '', livestreamId: rule.conditions.livestreamId || '' }); setTest(null); }}>Load redacted sample</button><button type="button" className="secondary-button" disabled={testing || busy || !valid || !canManage || (textEvent && streamMode !== 'any' && !rule.conditions.livestreamId)} onClick={() => void dryRun()}>Test rule</button></div>
      {testError ? <p role="alert" className="event-field-error">{testError}</p> : null}
      {test ? <div className="event-test-result" role="status"><strong>{test.matched ? 'Matched' : 'No match'}</strong><p>Source: {sourceName} · {families.find(f => f[0] === rule.eventType)?.[1]}</p><p>Actor: {test.actorLabel}{gift ? ' (purchaser)' : ''}</p><p>{test.calculation}</p><p>Repeat actor: {test.repeatActorPolicy === 'accumulate' ? 'Accumulate' : 'Skip existing'}</p><p>{test.action}</p></div> : null}
      </fieldset>
      {(!valid || (textEvent && streamMode !== 'any' && !rule.conditions.livestreamId)) ? <p className="event-field-error" role="status">Complete the highlighted fields before saving or testing.</p> : null}
      <button className="primary-button" disabled={busy || !canManage || !valid || (textEvent && streamMode !== 'any' && !rule.conditions.livestreamId)} type="submit">{busy ? 'Working…' : rule.enabled ? 'Save and enable' : 'Save paused rule'}</button>
    </>}
  </form>;
}
