...
export function AdminAccessBoundary({ children }: { children: ReactNode }) {
  const { loading, account, access, error, openAuth, signOut } = useAuth();
  ...
  if (!access.isAdmin) return <AccessFrame title="Admin access required" detail={`${account.displayName}, your account is active but does not have an Admin role.`} identity={account.email || account.username || account.displayName} action="Sign out" onAction={() => void signOut()} />;
  return <>{children}</>;
}

function AccessFrame({ title, detail, busy = false, identity, action, onAction }: { title: string; detail: string; busy?: boolean; identity?: string; action?: string; onAction?: () => void }) {
