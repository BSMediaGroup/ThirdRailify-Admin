...
function DocumentPreview({ template, preview, business }: { template: CommerceTemplate; preview: TemplatePreviewPayload | null; business: BusinessPayload }) {
  const rendered = preview?.preview || template; const variables = preview?.variables || {};
  return <aside className="document-preview-panel" aria-label={`${template.templateKey === "payment_receipt" ? "Receipt" : "Invoice"} preview`}><header><div><span className="document-sample-marker">SAMPLE · TEST · NOT ISSUED</span><small>{preview ? "Production renderer output" : "Preview not rendered"}</small></div><StatusChip state={preview ? "complete" : "unverified"} label={preview ? "Rendered" : "Render required"} /></header><article style={{ borderColor: rendered.accentColor }}><div className="document-preview-brand"><strong>{business.profile.tradingName || "Third Railify Official"}</strong><span>{template.templateKey === "payment_receipt" ? "PAYMENT RECEIPT" : "INVOICE / SALES DOCUMENT"}</span></div><div className="document-preview-body"><div className="document-preview-title"><div><small>TEST REFERENCE</small><strong>{variables.order_reference || "PREVIEW PENDING"}</strong></div><div><small>CUSTOMER</small><strong>{variables.customer_name || "Sample customer"}</strong></div></div><h3>{rendered.heading || "Document heading"}</h3><p>{rendered.introduction || "Render a preview to merge the approved variables."}</p><div className="document-line-items"><header><span>Item</span><span>Amount</span></header><div><strong>{variables.product_summary || "Sample line item"}</strong><span>{variables.order_total ? `${variables.order_total} ${variables.currency || "CAD"}` : "—"}</span></div></div><dl><Fact term="Subtotal" value={variables.order_total ? `${variables.order_total} ${variables.currency || "CAD"}` : "—"} /><Fact term="Shipping" value="Not supplied by sample fixture" /><Fact term="Tax" value="Not supplied by sample fixture" /><Fact term="Total" value={variables.order_total ? `${variables.order_total} ${variables.currency || "CAD"}` : "—"} /></dl>{rendered.bodyBlocks?.length > 0 && <div className="document-preview-blocks">{rendered.bodyBlocks.map((block, index) => <p key={`${index}-${block}`}>{block}</p>)}</div>}{rendered.supportText && <p className="document-preview-support">{rendered.supportText}</p>}</div><footer><span>{rendered.footer || business.readiness.documentIdentity.contactEmail || "Footer not configured"}</span><small>Preview only · no payment, persistence, token, email, provider order, or customer delivery</small></footer></article></aside>;
}
