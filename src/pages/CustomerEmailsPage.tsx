...
export function CustomerEmailsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSelected(key); setDraft(template); setPreview(null); setPreviewRevision("");
      if (template && template.validity?.state !== "invalid" && csrfToken) await renderPreview(template);
    } catch (reason) { if (sequence === statusSequence.current) { setStatusUnavailable(true); setError(errorMessage(reason, "Customer email authority is restricted or unavailable.")); } }
    finally { setBusy(""); stop(); }
  }, [csrfToken, renderPreview, startLoading]);

  useEffect(() => { void load(); return () => { statusSequence.current += 1; }; }, [load]);
  useEffect(() => {
    let pending = false;
    const refresh = async () => {
      if (pending || document.visibilityState === "hidden" || busy) return;
      pending = true;
      const sequence = ++statusSequence.current;
      try {
        const next = await getCustomerEmailsControlPlane();
        if (sequence === statusSequence.current) {
          // Operational facts refresh independently of the editor's persisted baseline.
          setPayload((previous) => previous ? { ...next, templates: previous.templates } : next);
          setStatusUnavailable(false);
        }
      } catch { if (sequence === statusSequence.current) setStatusUnavailable(true); }
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      setStatusUnavailable(false); setPayload(next); setDraft(saved); showToast("Template saved with a new server revision. No email was sent.", { title: "Email template saved" });
  ...
  return <div className="customer-emails-page">
    <EmailHero payload={statusUnavailable ? null : payload} unavailable={statusUnavailable} />
    ...
    {!payload && !error ? <EmailState title="Loading customer email authority" detail="Reading templates, sender configuration, dependencies, and persisted delivery evidence." /> : payload ? <>
      ...
      {statusUnavailable ? <EmailState title="Status unavailable" detail="Operational status could not be refreshed. Unsaved template edits are preserved." /> : <SenderProvider payload={payload} />}
          ...
          <div className="customer-email-editor-shell">
            ...
            <div className="customer-email-editor-grid">
              <form className="customer-email-form" onSubmit={(event) => void save(event)} noValidate>
                ...
                <div className="customer-email-field-row"><EditorField label="Template state" hint="Ready is a template state, not a production-send switch."><select value={draft.status} onChange={(event) => change("status", event.target.value as CommerceTemplate["status"])}><option value="draft">Draft</option><option value="ready">Ready for eligible delivery</option><option value="disabled">Disabled</option></select></EditorField><EditorField label="Template enabled" hint="Still subordinate to the global delivery gate."><select value={draft.enabled ? "enabled" : "disabled"} onChange={(event) => change("enabled", event.target.value === "enabled")}><option value="disabled">Disabled</option><option value="enabled">Enabled</option></select></EditorField></div>
      ...
      {!statusUnavailable && <DependenciesAndBoundary payload={payload} />}
      <DeliveryEvidence payload={payload} />
      {!statusUnavailable && <AdvancedEvidence payload={payload} />}
    </> : null}
  </div>;
}

function deliveryStatus(payload: CustomerEmailsPayload | null, unavailable = false) {
  const state = payload?.readiness.delivery?.state;
  if (!payload || !state || state === "unavailable") return { label: unavailable || payload ? "Status unavailable" : "Loading", tone: "quiet" as const, detail: "Reading current authenticated sending authority." };
  if (state === "disabled") return { label: "Production sends disabled", tone: "bad" as const, detail: payload.readiness.delivery.blockers.join(". ") };
  if (state === "action_required") return { label: "Sending enabled — action required", tone: "warn" as const, detail: payload.readiness.delivery.blockers.join(". ") };
  return { label: "Production sends enabled", tone: "good" as const, detail: "Production delivery active. Eligible lifecycle events trigger delivery; this status does not claim a recorded delivery." };
}
function verificationLabel(payload: CustomerEmailsPayload) {
  const evidence = payload.verification;
  if (evidence?.status === "verified") return evidence.observedAt ? `Verified — last recorded ${formatTimestamp(evidence.observedAt)}` : "Verified — recorded evidence";
  return evidence?.status === "failed" ? "Verification failed — recorded evidence" : "Verification evidence unavailable";
}
function lifecycleLabel(payload: CustomerEmailsPayload) { return payload.readiness.productionLifecycleImplemented ? "Implemented — queued after a completed LIVE store capture; shipment updates use the existing shipment lifecycle" : "Not implemented"; }
function EmailHero({ payload, unavailable = false }: { payload: CustomerEmailsPayload | null; unavailable?: boolean }) {
  const status = deliveryStatus(payload, unavailable);
  return <section className="customer-email-hero" aria-labelledby="customer-emails-title"><div><div className="area-icon"><AdminIcon name="emails" size={28} /></div><p className="eyebrow">Authoritative customer communications</p><h1 id="customer-emails-title">Customer emails</h1><p>Transactional sender readiness, revisioned lifecycle templates, canonical previews, and persisted delivery evidence.</p><div className="customer-email-hero__chips"><EmailChip tone={payload?.provider.configured ? "good" : "quiet"}>{payload ? `Sender ${payload.provider.configured ? "configured" : "not configured"}` : status.label}</EmailChip><EmailChip tone={payload?.readiness.configurationReady ? "good" : "quiet"}>Templates {payload ? `${payload.readiness.configuredTemplates}/${payload.readiness.totalTemplates}` : "Loading"}</EmailChip><EmailChip tone={status.tone}>{status.label}</EmailChip></div></div><div className="customer-email-readiness-mark"><span>Production delivery status</span><strong>{status.label}</strong><p>{status.detail}</p></div></section>;
}

function SenderProvider({ payload }: { payload: CustomerEmailsPayload }) {
  return <section className="email-control-section" aria-labelledby="sender-provider-title"><SectionHeading eyebrow="Server projection" title="Sender identity & provider" id="sender-provider-title" detail="These values come from the server environment and Business Information. Provider secrets and raw responses never reach the browser." /><div className="customer-email-summary-grid"><SummaryMetric label="Provider" value={payload.provider.name} state={payload.provider.configured ? "good" : "warn"} /><SummaryMetric label="Sender" value={payload.sender.fromAddressConfigured ? "Configured" : "Action required"} state={payload.sender.fromAddressConfigured ? "good" : "warn"} /><SummaryMetric label="Reply-To" value={payload.sender.replyToConfigured ? "Configured" : "Not configured"} state={payload.sender.replyToConfigured ? "good" : "warn"} /><SummaryMetric label="Templates" value={`${payload.readiness.configuredTemplates}/${payload.readiness.totalTemplates} configured`} state={payload.readiness.configuredTemplates >= payload.readiness.minimumReadyTemplates ? "good" : "warn"} /><SummaryMetric label="Dispatch capability" value={payload.readiness.delivery.dispatchConfigured == null ? "Status unavailable" : payload.readiness.delivery.dispatchConfigured ? "Configured" : "Action required"} state={payload.readiness.delivery.dispatchConfigured ? "good" : "warn"} /><SummaryMetric label="Customer sends" value={deliveryStatus(payload).label} state={deliveryStatus(payload).tone} /></div><div className="customer-email-card-grid"><article className="customer-email-card"><header><div><p className="eyebrow">Configured in server environment</p><h3>Sender identity</h3></div><EmailChip tone={payload.sender.fromAddressConfigured ? "good" : "warn"}>{payload.sender.fromAddressConfigured ? "Configured" : "Incomplete"}</EmailChip></header><dl><Fact term="From display name" value={payload.sender.fromDisplayName || "Not configured"} /><Fact term="From address" value={payload.sender.fromAddress || "Not configured"} /><Fact term="Reply-To" value={payload.sender.replyToAddress || "Not configured"} /><Fact term="Sending domain" value={payload.sender.sendingDomain || "Not configured"} /><Fact term="Domain verification" value={verificationLabel(payload)} /></dl><p>Merchant-facing name and support contact remain owned by Business Information.</p><Link to="/commerce/business">Manage Business Information <AdminIcon name="arrow" size={14} /></Link></article><article className="customer-email-card"><header><div><p className="eyebrow">Transactional provider</p><h3>{payload.provider.name}</h3></div><EmailChip tone={payload.provider.configured ? "good" : "warn"}>{payload.provider.configured ? "Configured" : "Not configured"}</EmailChip></header><dl><Fact term="Credential" value={payload.provider.credentialConfigured ? "Configured server-side" : "Not configured"} /><Fact term="Sender" value={payload.provider.senderConfigured ? "Configured server-side" : "Not configured"} /><Fact term="Reply-To" value={payload.provider.replyToConfigured ? "Configured server-side" : "Not configured"} /><Fact term="External verification" value={verificationLabel(payload)} /><Fact term="Provider health" value="Not queried" /><Fact term="Last persisted success" value={payload.provider.lastSuccessful ? formatTimestamp(payload.provider.lastSuccessful.sentAt || payload.provider.lastSuccessful.updatedAt) : "No recorded delivery"} /><Fact term="Last persisted failure" value={payload.provider.lastFailed ? formatTimestamp(payload.provider.lastFailed.updatedAt) : "No recorded failure"} /></dl><p>No Connect, reconnect, domain-check, or provider-send action is available here.</p></article></div></section>;
}
...
function AdvancedEvidence({ payload }: { payload: CustomerEmailsPayload }) {
  return <details className="customer-email-advanced"><summary>Advanced / technical evidence</summary><div><FactList title="Provider & renderer" facts={[["Authority", payload.authority], ["Provider", payload.provider.name], ["Credential", payload.provider.credentialConfigured ? "Configured server-side" : "Not configured"], ["Domain verification", verificationLabel(payload)], ["Evidence source", payload.verification?.source || "Unavailable"], ["Evidence domain", payload.verification?.domain || "Unavailable"], ["Domain ID", payload.verification?.domainId || "Unavailable"], ["Provider query on this page", "Not performed"], ["Production delivery", deliveryStatus(payload).label], ["Delivery conditions", deliveryStatus(payload).detail], ["Global customer-send gate", payload.readiness.customerSendsEnabled == null ? "Status unavailable" : payload.readiness.customerSendsEnabled ? "Enabled" : "Disabled"], ["Lifecycle trigger", lifecycleLabel(payload)]]} /><FactList title="Lifecycle eligibility" facts={(payload.readiness.delivery?.events || []).map((event) => [humanize(event.templateKey), event.eligible ? "Eligible for delivery" : event.blockers.join(". ") || "Global sending disabled"])} /><FactList title="Delivery ledger" facts={[["Idempotency", payload.deliveries.idempotency.implemented ? "Implemented" : "Unavailable"], ["Key authority", payload.deliveries.idempotency.authority], ["Browser-generated keys", "Not trusted"], ["Retries from page", "Unavailable"], ["TEST records", String(payload.deliveries.counts.test)], ["LIVE records", String(payload.deliveries.counts.live)]]} /><FactList title="Safety assertions" facts={[["Preview mutates", "No"], ["Provider calls on read", "No"], ["Provider calls on preview", "No"], ["Test send exposed", "No"], ["Production control exposed", "No"], ["PayPal dependency", payload.dependencies.paypalRequired ? "Unexpected" : "Not required"]]} /></div></details>;
}
