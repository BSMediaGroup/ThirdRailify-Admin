...
function OrderDetailView({ order, loading, onClose }: { order: CommerceOrderDetail | null; loading: boolean; onClose: () => void }) {
  ...
  return <section className={`order-detail ${order.test ? "is-test" : "is-live"}`}>
    ...
    <div className="order-detail__grid"><Section title="Customer" eyebrow="Protected customer data">{order.customer.available ? <dl><Fact term="Name" value={order.customer.name || "Not recorded"} /><Fact term="Email" value={order.customer.email || "Not recorded"} /><Fact term="Phone" value={order.customer.phone || "Not recorded"} /></dl> : order.delivery.recipientConfigured ? <Missing title="Delivery recipient encrypted" text="A server-encrypted fulfillment snapshot exists. Name, street, postal code, phone, and encryption material are not exposed in this projection." /> : <Missing title="No customer PII stored" text="This legacy order has no customer or delivery snapshot." />}</Section><Section title="Financial breakdown" eyebrow="Stored minor-unit values"><dl><MoneyFact term="Line-item subtotal" value={order.financial.subtotalAmount} currency={order.currencyCode} /><Fact term="Discount" value="Not recorded" /><Fact term="Customer shipping" value={order.financial.shippingAmount === null ? "Not recorded" : formatMoney(order.financial.shippingAmount, order.currencyCode)} /><Fact term="Customer tax" value="Not recorded" /><MoneyFact term="Order total" value={order.financial.totalAmount} currency={order.currencyCode} /><MoneyFact term="Refund" value={order.financial.refundAmount} currency={order.currencyCode} /><Fact term="Net after refund" value={order.financial.netAmount === null ? "Not safely derivable" : formatMoney(order.financial.netAmount, order.currencyCode)} /></dl></Section></div>
