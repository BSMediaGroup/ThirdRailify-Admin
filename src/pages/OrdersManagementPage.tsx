...
export function OrdersManagementPage() {
  ...
  const closeOrder = useCallback(() => { setSearchParams((current) => { const next = new URLSearchParams(current); next.delete("order"); return next; }); setDetail(null); }, [setSearchParams]);
  const openOrder = useCallback((orderId: string) => { const next = new URLSearchParams(searchParams); next.set("order", orderId); setSearchParams(next); }, [searchParams, setSearchParams]);
  useEffect(() => {
    if (!selectedId) { setDetail(null); setDetailLoading(false); return; }
    let active = true; setDetail(null); setDetailLoading(true); setError("");
    void getCommerceOrder(selectedId).then((result) => { if (active) setDetail(result.order); }).catch((reason) => { if (active) { setError(errorMessage(reason, "The order detail is unavailable.")); closeOrder(); } }).finally(() => { if (active) setDetailLoading(false); });
    return () => { active = false; };
  }, [closeOrder, selectedId]);
...
function OrderDetailView({ order, loading, onClose }: { order: CommerceOrderDetail | null; loading: boolean; onClose: () => void }) {
  ...
  return <section className={`order-detail ${order.test ? "is-test" : "is-live"}`}>
    ...
    <div className="order-detail__grid"><Section title="Customer" eyebrow="Protected customer relationship">{order.customer.linked ? <><dl><Fact term="Type" value={order.customer.kind === "account" ? "Account-backed Customer" : "Guest Customer"} /><Fact term="Current contact" value={order.customer.contact ? `${order.customer.contact.name} · ${order.customer.contact.email}` : "Not recorded"} /><Fact term="Checkout snapshot" value={order.customer.snapshot ? `${order.customer.snapshot.name || "Name not recorded"} · ${order.customer.snapshot.email || "Email not recorded"}` : "Not recorded"} /><Fact term="Phone" value={order.customer.phone || "Not recorded"} /></dl><div className="order-customer-links"><Link className="secondary-button" to={`/customers?customer=${encodeURIComponent(order.customer.id || "")}`}>Open Customer</Link>{order.customer.account && <Link className="secondary-button" to={`/access?account=${encodeURIComponent(order.customer.account.id)}`}>Open Account</Link>}</div></> : <Missing title="Customer not linked / legacy order" text={order.delivery.recipientConfigured ? "A historical encrypted delivery snapshot exists, but this order predates Customer authority." : "This order predates Customer authority and has no linked customer or delivery snapshot."} />}</Section><Section title="Financial breakdown" eyebrow="Stored minor-unit values"><dl><MoneyFact term="Line-item subtotal" value={order.financial.subtotalAmount} currency={order.currencyCode} /><Fact term="Discount" value="Not recorded" /><Fact term="Customer shipping" value={order.financial.shippingAmount === null ? "Not recorded" : formatMoney(order.financial.shippingAmount, order.currencyCode)} /><Fact term="Customer tax" value="Not recorded" /><MoneyFact term="Order total" value={order.financial.totalAmount} currency={order.currencyCode} /><MoneyFact term="Refund" value={order.financial.refundAmount} currency={order.currencyCode} /><Fact term="Net after refund" value={order.financial.netAmount === null ? "Not safely derivable" : formatMoney(order.financial.netAmount, order.currencyCode)} /></dl></Section></div>
    <Section title="Delivery & shipping" eyebrow="Encrypted historical snapshot">{order.delivery.available ? <dl><Fact term="Recipient" value={order.delivery.recipientConfigured ? "Configured / encrypted" : "Not configured"} /><Fact term="Destination" value={[order.delivery.destinationCountryCode, order.delivery.destinationRegionCode].filter(Boolean).join(" / ") || "Not recorded"} /><Fact term="Method" value={order.delivery.method || "Not recorded"} /><Fact term="Shipping amount" value={order.delivery.amount === null ? "Not recorded" : formatMoney(order.delivery.amount, order.delivery.currencyCode || order.currencyCode)} /><Fact term="Strategy" value={order.delivery.strategy ? humanize(order.delivery.strategy) : "Not recorded"} /><Fact term="Quote reference" value={order.delivery.quoteReference || "Not recorded"} /><Fact term="Quoted" value={formatTimestamp(order.delivery.quotedAt)} /><Fact term="Address verification" value="Not externally verified" /></dl> : <Missing title={order.test ? "Shipping not captured / legacy TEST evidence" : "Shipping not captured"} text="This order predates the delivery snapshot authority or did not require physical shipping." />}</Section>
