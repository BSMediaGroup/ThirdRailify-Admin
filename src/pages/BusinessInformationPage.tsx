...
function Sensitive({ title, configured, confirmed = false, mask, replacing, onToggle, children }: { title: string; configured: boolean; confirmed?: boolean; mask: string; replacing: boolean; onToggle: () => void; children: ReactNode }) { return <div className="business-sensitive-row"><div><strong>{title}</strong><span>{configured ? mask || "Encrypted value configured" : "Not configured"}</span></div><StatusChip state={configured ? "complete" : "not_configured"} label={configured ? confirmed ? "Owner confirmed" : "Configured" : undefined} /><button className="button-link" type="button" onClick={onToggle}>{replacing ? "Cancel" : configured ? "Replace" : "Add"}</button>{replacing && <div className="business-sensitive-fields">{children}</div>}</div>; }
