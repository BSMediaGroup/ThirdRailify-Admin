...
import { normalizeCountry, normalizeRegion, postalLabel } from "../address/geography";
...
export function BusinessInformationPage() {
  ...
  return <div className="business-information-page">
    ...
    <StoreLaunchWorkspace profileDraft={dirty ? mutationPayload(form,profile.revision) : undefined} onActivated={()=>void load()} />
    <form className="business-settings-form" onSubmit={(event) => void submit(event)} noValidate>
      <div className="business-settings-grid">
        ...
        <Section id="legal-merchant-details" eyebrow="Sensitive · encrypted" title="Legal / merchant details" description="Values are never prefilled. Replace only when an intentional update is required.">
          <Sensitive title="Legal registered name" configured={profile.private.legalBusinessNameStored} mask={profile.private.legalBusinessNameMasked} replacing={form.replaceLegalName} onToggle={() => update("replaceLegalName", !form.replaceLegalName)}><Field label="Replacement legal name" hint="Encrypted at rest; manually entered and owner-entered." error={fieldErrors.legalBusinessName}><input value={form.legalBusinessName} onChange={(event) => update("legalBusinessName", event.target.value)} autoComplete="organization" maxLength={240} aria-invalid={Boolean(fieldErrors.legalBusinessName)} /></Field></Sensitive>
          ...
          <Sensitive title="Private business phone" configured={profile.private.privatePhoneStored} confirmed={profile.attestation.ownerConfirmed} mask={profile.private.privatePhoneMasked} replacing={form.replacePrivatePhone} onToggle={() => update("replacePrivatePhone", !form.replacePrivatePhone)}><Field label="Replacement private phone" hint="Encrypted merchant record; transaction disclosure only after explicit launch authorization." error={fieldErrors.privatePhone}><input type="tel" value={form.privatePhone} onChange={(event) => update("privatePhone", event.target.value)} autoComplete="off" maxLength={80} aria-invalid={Boolean(fieldErrors.privatePhone)} /></Field></Sensitive>
          <Privacy tone="sensitive" label="SENSITIVE" text="Only configured state and bounded masks return to this browser." />
        </Section>
        <Section id="business-address" eyebrow="Operator-owned metadata" title="Business address" description="Business metadata and transaction-disclosure readiness are separate. Country and region names map to canonical machine codes; this is not a postal-verification system.">
          ...
          <div className="business-address-block business-address-block--private"><header><div><span>Legal business address</span><small>SENSITIVE · encrypted; stored address is never returned as plaintext.</small></div><StatusChip state={profile.private.privateAddressStored ? "complete" : "not_configured"} label={profile.private.privateAddressStored ? profile.attestation.ownerConfirmed ? "Operator attested" : "Configured" : undefined} /></header><button className="secondary-button business-replace-button" type="button" onClick={() => update("replacePrivateAddress", !form.replacePrivateAddress)}>{form.replacePrivateAddress ? "Cancel replacement" : profile.private.privateAddressStored ? "Replace encrypted address" : "Add legal address"}</button>{form.replacePrivateAddress && <AddressFields kind="private" form={form} errors={fieldErrors} update={update} />}</div>
...
function Sensitive({ title, configured, confirmed = false, mask, replacing, onToggle, children }: { title: string; configured: boolean; confirmed?: boolean; mask: string; replacing: boolean; onToggle: () => void; children: ReactNode }) { return <div className="business-sensitive-row"><div><strong>{title}</strong><span>{configured ? mask || "Encrypted value configured" : "Not configured"}</span></div><StatusChip state={configured ? "complete" : "not_configured"} label={configured ? confirmed ? "Operator attested" : "Configured" : undefined} /><button className="button-link" type="button" onClick={onToggle}>{replacing ? "Cancel" : configured ? "Replace" : "Add"}</button>{replacing && <div className="business-sensitive-fields">{children}</div>}</div>; }
...
function AddressFields({ kind, form, errors, update }: { kind: "business" | "private"; form: BusinessDraft; errors: Record<string, string>; update: BusinessDraftUpdater }) {
  ...
  return <div className={`business-field-grid business-address-fields is-${prefix}`}><Field wide label="Address line 1" error={errors[keys.line1]}><input value={values.line1} onChange={(event) => update(keys.line1, event.target.value)} autoComplete={kind === "private" ? "street-address" : "off"} maxLength={180} aria-invalid={Boolean(errors[keys.line1])} /></Field><Field wide label="Address line 2" hint="Optional free text."><input value={values.line2} onChange={(event) => update(keys.line2, event.target.value)} autoComplete={kind === "private" ? "address-line2" : "off"} maxLength={180} /></Field><Field label="City / locality" error={errors[keys.city]}><input value={values.city} onChange={(event) => update(keys.city, event.target.value)} autoComplete="address-level2" maxLength={120} aria-invalid={Boolean(errors[keys.city])} /></Field><CountryRegionFields idPrefix={`${prefix}-address`} countryCode={values.country} region={values.province} onCountryChange={(next) => update(keys.country, next)} onRegionChange={(next) => update(keys.province, next)} countryError={errors[keys.country]} regionError={errors[keys.province]} /><Field label={postalLabel(values.country)} error={errors[keys.postal]}><input value={values.postal} onChange={(event) => update(keys.postal, event.target.value)} autoComplete="postal-code" maxLength={64} aria-invalid={Boolean(errors[keys.postal])} /></Field></div>;
}
...
function addressConfigured(address: BusinessAddress) { return Object.values(address).some(Boolean); }
function stateLabel(state: BusinessReadinessState) { return ({ complete: "Complete", partial: "Partial", action_required: "Action required", incomplete: "Action required", not_configured: "Not configured", not_required: "Not required", unverified: "Configured", disabled: "Disabled" } as const)[state]; }
