...
function NormalEditor({ config, setConfig }: EditorProps) {
  ...
  return <section className="banner-editor__panel" aria-labelledby="normal-banner-title">
    ...
    <div className="banner-editor__controls"><label><span>Presentation mode</span><select value={config.normal.mode} onChange={(event) => update({ mode: event.target.value as BannerConfig["normal"]["mode"] })}><option value="static">Static</option><option value="ticker">Horizontal ticker</option><option value="crossfade">Crossfade</option></select></label><label><span>Animation speed</span><select value={config.normal.speed} onChange={(event) => update({ speed: event.target.value as BannerConfig["normal"]["speed"] })}><option value="slow">Slow</option><option value="normal">Normal</option><option value="fast">Fast</option></select></label><label className="banner-check is-wide"><input type="checkbox" checked={config.normal.dismissible} onChange={(event) => update({ dismissible: event.target.checked })} /><span>Allow visitors to dismiss this banner with a close button</span></label></div>
...
function NormalPreview({ config }: { config: BannerConfig }) {
  ...
  const content = (message: BannerMessage, duplicate = false) => <span aria-hidden={duplicate || undefined}>{message.text}{message.ctaLabel && <b>{message.ctaLabel}<PreviewArrowIcon /></b>}<i aria-hidden="true" /></span>;
