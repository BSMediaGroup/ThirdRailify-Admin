  ...
  return <>
    ...
    {!payload && !error ? <CommerceState>Loading truthful commerce status…</CommerceState> : payload ? <>
      ...
      <section className="commerce-section" aria-labelledby="readiness-title"><SectionTitle id="readiness-title" eyebrow="Derived from actual configuration" title="Production readiness" />
        <div className={`commerce-callout ${launch?.ready ? "is-connected" : "is-unavailable"}`}><AdminIcon name="shield" /><div><strong>{launch?.ready ? "All pre-activation hard gates are ready" : "Production activation remains blocked"}</strong><p>{launch ? (launch.ready ? "The atomic activation control above may enable future LIVE PayPal merchandise orders without creating a transaction." : `${launch.hardGates.filter((gate) => !gate.ready).length} hard gate${launch.hardGates.filter((gate) => !gate.ready).length === 1 ? " remains" : "s remain"}; exact current evidence is listed above.`) : "Launch authority is unavailable."} Stripe TEST acceptance remains historical evidence and is not a PayPal launch blocker.</p></div></div>
