  ...
  return <>
    ...
    {!payload && !error ? <CommerceState>Loading truthful commerce status…</CommerceState> : payload ? <>
      ...
      <section className="commerce-section" aria-labelledby="readiness-title"><SectionTitle id="readiness-title" eyebrow="Derived from actual configuration" title="Production readiness" />
        <div className={`commerce-callout ${payload.readiness?.productionReady ? "is-connected" : "is-unavailable"}`}><AdminIcon name="shield" /><div><strong>{payload.readiness?.productionReady ? "Production commerce is ready" : "Production commerce remains blocked"}</strong><p>PayPal LIVE credentials, merchant readback, webhook registration, and explicit capture gates are required. Stripe TEST acceptance remains historical evidence and is not a PayPal launch blocker.</p></div></div>
