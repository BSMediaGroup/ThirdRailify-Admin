...
const COMMERCE_WORKSPACES = [
  ...
  { to: "/commerce/emails", eyebrow: "Lifecycle templates", title: "Customer emails", text: "Structured templates, authoritative sending status, and delivery history.", icon: "emails" },
  ...
  return <>
    ...
    {!payload && !error ? <CommerceState>Loading truthful commerce status…</CommerceState> : payload ? <>
      ...
      <section className="commerce-section" aria-labelledby="readiness-title"><SectionTitle id="readiness-title" eyebrow="Derived from actual configuration" title="Production readiness" />
        <div className={`commerce-callout ${launch?.ready ? "is-connected" : "is-unavailable"}`}><AdminIcon name="shield" /><div><strong>{launch?.operationalState === "active" ? "Production checkout is operational" : launch?.state === "active" ? "Active store needs attention" : launch?.ready ? "All pre-activation hard gates are ready" : "Production activation remains blocked"}</strong><p>{launch ? (launch.operationalState === "active" ? "PayPal LIVE checkout and fulfillment are enabled. Each order revalidates cart, destination, shipping, tax, and agreement authority before payment." : launch.ready ? "The atomic activation control above may enable future LIVE PayPal merchandise orders without creating a transaction." : `${launch.hardGates.filter((gate) => !gate.ready).length} hard gate${launch.hardGates.filter((gate) => !gate.ready).length === 1 ? " remains" : "s remain"}; exact current evidence is listed above.`) : "Launch authority is unavailable."} Stripe TEST acceptance remains historical evidence and is not a PayPal launch blocker.</p></div></div>
        ...
        <div className="commerce-metric-grid"><Metric label="Preferred provider" value="PayPal" /><Metric label="PayPal LIVE" value={payload.readiness?.domains.payments.ready ? "Ready" : "Blocked"} /><Metric label="Store checkout" value={payload.readiness?.domains.checkout.details.normalCheckoutEnabled === true ? "Enabled" : "Disabled"} /><Metric label="Donations" value={payload.readiness?.domains.checkout.details.donationsEnabled === true ? "Enabled" : "Disabled"} /><Metric label="Stripe" value="Configured / disabled" /></div>
      </section>
    </> : null}
  </>;
}
...
export function PaymentsPayoutsPage() {
  ...
  useEffect(() => { void load(); }, [load]);
  const activation = payload?.productionActivation;
  return <>
    <section className="payments-hero commerce-heading" aria-labelledby="payments-title">
      <div className="payments-hero__copy"><div className="area-icon"><AdminIcon name="payments" size={28} /></div><p className="eyebrow">Direct merchant control plane</p><h1 id="payments-title">Payments &amp; payouts</h1><p>PayPal is the preferred provider for store purchases and one-time donations. Stripe remains configured, historically readable, and explicitly disabled.</p><div className="payments-hero__chips"><PaymentStateChip state={payload?.paypal.credentialConfigured ? "configured" : "unverified"} label={payload?.paypal.credentialConfigured ? "PayPal configured" : "PayPal setup required"} /><PaymentStateChip state="verified" label="Direct merchant" /><PaymentStateChip state={activation?.state || "unverified"} label={activation ? productionActivationLabel(activation.state) : "Reading production authority"} /></div></div>
      <div className="payments-flow" aria-label="Direct merchant payment architecture"><div><span>Store &amp; donations</span><strong>PayPal v6</strong></div><AdminIcon name="arrow" /><div><span>Direct merchant</span><strong>Orders v2 / CAPTURE</strong></div><AdminIcon name="arrow" /><div><span>Local evidence</span><strong>Commerce D1</strong></div><small>Server-created orders · server capture · one verified webhook pipeline</small></div>
    </section>
    {error && <div className="admin-alert" role="alert">{error}</div>}
    {!payload && !error ? <CommerceState><strong>Loading payments authority…</strong><span>Reading bounded Commerce D1 and server configuration projections.</span></CommerceState> : payload ? <PaymentsControlPlane payload={payload} /> : null}
  </>;
}

function PaymentsControlPlane({ payload }: { payload: PaymentsControlPlanePayload }) {
  ...
  return <div className="payments-workspace">
    ...
    <section className="payments-summary" aria-label="Payments summary"><PaymentMetric label="Preferred" value="PayPal" state="verified" /><PaymentMetric label="LIVE API" value={payload.paypal.live.clientIdConfigured && payload.paypal.live.clientSecretConfigured ? "Configured" : "Setup required"} state={payload.paypal.live.clientIdConfigured && payload.paypal.live.clientSecretConfigured ? "configured" : "unverified"} /><PaymentMetric label="LIVE webhook" value={payload.paypal.live.webhookIdConfigured ? "Configured" : "Setup required"} state={payload.paypal.live.webhookIdConfigured ? "configured" : "unverified"} /><PaymentMetric label="Store" value={payload.paypal.shopCheckoutEnabled ? "Enabled" : "Disabled"} state={payload.paypal.shopCheckoutEnabled ? "configured" : "disabled"} /><PaymentMetric label="Donations" value={payload.paypal.donationsEnabled ? "Enabled" : "Disabled"} state={payload.paypal.donationsEnabled ? "configured" : "disabled"} /><PaymentMetric label="Stripe" value="Configured / disabled" state="disabled" /></section>

    <section className="commerce-section" aria-labelledby="paypal-provider-title"><SectionTitle id="paypal-provider-title" eyebrow="Preferred processor" title="PayPal direct merchant" /><div className="payments-primary-grid">
      <article className="payments-panel payments-provider-card has-provider-feature"><img className="provider-feature-icon" src={paypalFeatureIcon} alt="" aria-hidden="true" /><header><div><p className="eyebrow">Preferred · Orders API v2</p><h3>PayPal</h3></div><PaymentStateChip state={payload.paypal.credentialConfigured ? "configured" : "unverified"} label={payload.paypal.credentialConfigured ? "Configured" : "Setup required"} /></header><p>One direct-merchant CAPTURE integration serves physical store orders and one-time donations through separate local authorities.</p><dl><Fact term="Sandbox credentials" value={payload.paypal.sandbox.clientIdConfigured && payload.paypal.sandbox.clientSecretConfigured ? "Configured" : "Missing"} /><Fact term="Sandbox OAuth" value={payload.paypal.sandbox.oauthVerified ? "Verified" : "Unverified"} /><Fact term="Sandbox webhook" value={payload.paypal.sandbox.webhookReadbackVerified ? "Configured / read back" : "Unconfigured"} /><Fact term="Sandbox store acceptance" value={payload.paypal.sandbox.storeAcceptance === "passed" ? "Passed" : "Not run"} /><Fact term="Sandbox donation acceptance" value={payload.paypal.sandbox.donationAcceptance === "passed" ? "Passed" : "Not run"} /><Fact term="LIVE credentials" value={payload.paypal.live.clientIdConfigured && payload.paypal.live.clientSecretConfigured ? "Configured" : "Missing"} /><Fact term="LIVE OAuth" value={payload.paypal.live.oauthVerified ? "Verified" : "Unverified"} /><Fact term="LIVE webhook" value={payload.paypal.live.webhookReadbackVerified ? "Configured / read back" : "Unconfigured"} /><Fact term="Store checkout" value={payload.paypal.shopCheckoutEnabled ? "Enabled" : "Disabled"} /><Fact term="Donations" value={payload.paypal.donationsEnabled ? "Enabled" : "Disabled"} /></dl><div className="payments-copy-actions">{payload.paypal.webhookUrl && <CopyValueButton label="Copy webhook URL" value={payload.paypal.webhookUrl} />}{payload.paypal.setupCommand && <CopyValueButton label="Copy local setup command" value={payload.paypal.setupCommand} />}</div><details className="payments-technical"><summary>Webhook event set</summary><p>{payload.paypal.webhookEvents.join(" · ")}</p><p className="commerce-action-note">This read-only page makes no PayPal API call and never projects credentials, OAuth tokens, or raw Webhook IDs.</p></details></article>
      <article className="payments-panel payments-provider-card has-provider-feature"><img className="provider-feature-icon" src={stripeFeatureIcon} alt="" aria-hidden="true" /><header><div><p className="eyebrow">Retained card-processing option</p><h3>{stripe.displayName}</h3></div><PaymentStateChip state="disabled" label="CONFIGURED / DISABLED" /></header><p>Stripe remains fully implemented and its historical TEST evidence is preserved. It is non-preferred, disabled, and excluded from current PayPal activation readiness.</p><dl><Fact term="Integration" value="Direct merchant" /><Fact term="Current use" value="Disabled" /><Fact term="New Checkout Sessions" value="Prohibited" /><Fact term="TEST acceptance" value={evidence ? "Preserved" : "Unverified"} /><Fact term="Restoration" value="Requires a separately authorized milestone" /></dl></article>
      <article className={`payments-panel production-activation is-${activation.state}`}><header><div><p className="eyebrow">Canonical launch authority</p><h3>Production activation</h3></div><PaymentStateChip state={activation.state} label={productionActivationLabel(activation.state)} /></header><p>{productionActivationCopy(activation.state)}</p><div className="activation-status-list"><ActivationStatusRow label="Public checkout" state={activation.checkout.enabled ? "enabled" : "disabled"} status={activation.checkout.enabled ? "Enabled" : "Disabled"} detail="Public order entry requires both the global checkout and PayPal store authorities." /><ActivationStatusRow label="Live payment capture" state={activation.livePayments.enabled ? "enabled" : "disabled"} status={activation.livePayments.enabled ? "Enabled" : "Disabled"} detail="Merchandise capture requires both the global LIVE and PayPal LIVE authorities." /><ActivationStatusRow label="Fulfillment submission" state={activation.fulfillment.enabled ? "enabled" : "disabled"} status={activation.fulfillment.enabled ? "Enabled" : "Disabled"} detail="Paid LIVE orders may enter the canonical fulfillment workflow." /><ActivationStatusRow label="Emergency pause" state={activation.emergencyPaused ? "paused" : "clear"} status={activation.emergencyPaused ? "Active" : "Clear"} detail={activation.emergencyPaused ? "Production commerce is blocked by the emergency authority." : "No emergency pause is active."} /><ActivationStatusRow label="Controlled TEST checkout" state="closed" status="Closed · test only" detail={`${activation.state === "active" ? "Isolated acceptance harness. Intentionally closed after production activation." : "Isolated acceptance harness. Closed and not a production dependency."}${activation.controlledTestCheckout.acceptanceEvidence === "preserved" ? " Acceptance evidence: preserved." : ""}`} /></div><div className="payments-lock-note"><AdminIcon name="shield" /><span>Read-only projection · canonical commerce authority</span></div></article>
    </div></section>
    ...
    <section className="commerce-section" aria-labelledby="methods-payouts-title"><SectionTitle id="methods-payouts-title" eyebrow="Provider boundaries" title="Payment methods &amp; activity" /><div className="payments-boundary-grid"><article className="payments-panel"><header><div><p className="eyebrow">Standard PayPal experience</p><h3>Payment methods</h3></div></header><div className="payment-method-list">{payload.paymentMethods.map((method) => <div key={method.id}><PaymentMethodMark id={method.id} /><div><strong>{method.label}</strong><p>{method.detail}</p></div><PaymentStateChip state={method.state} /></div>)}</div><p className="payments-lock-note">PayPal eligibility controls legitimate funding presentation. Advanced Card Fields are not enabled.</p></article><article className="payments-panel"><header><div><p className="eyebrow">Provider-neutral ledger</p><h3>PayPal attempts</h3></div><PaymentStateChip state={payload.paypal.attempts.length ? "configured" : "unverified"} /></header>{payload.paypal.attempts.length ? <dl>{payload.paypal.attempts.map((row) => <Fact key={`${row.environment}-${row.kind || "payment"}-${row.normalized_state}`} term={`${row.environment.toUpperCase()} / ${humanize(row.kind || "payment")} / ${humanize(row.normalized_state)}`} value={String(row.count)} />)}</dl> : <p>No PayPal payment attempts are stored.</p>}</article><article className="payments-panel"><header><div><p className="eyebrow">Separate non-fulfillment authority</p><h3>Donations</h3></div><PaymentStateChip state={payload.paypal.donationsEnabled ? "configured" : "disabled"} /></header>{payload.paypal.donations.length ? <dl>{payload.paypal.donations.map((row) => <Fact key={`${row.environment}-${row.status}`} term={`${row.environment.toUpperCase()} / ${humanize(row.status)}`} value={`${String(row.count)} · ${formatCad(Number(row.amount))}`} />)}</dl> : <p>No PayPal donations are stored. Donations never enter Printful fulfillment.</p>}</article></div></section>
    ...
    <details className="payments-technical payments-technical--page"><summary>Advanced architecture and evidence boundaries</summary><div className="payments-technical-grid"><FactList title="Payment architecture" facts={[["Preferred checkout", "PayPal Orders API v2"], ["Intent", "CAPTURE"], ["Merchant model", "Direct merchant"], ["Store authority", "Customer + Order + payment attempt"], ["Donation authority", "Donation + payment attempt"], ["Stripe", "Retained / disabled"], ["Multiparty platform", "Not used"], ["Advanced Card Fields", "Not enabled"]]} /><FactList title="Preserved Stripe webhook evidence" facts={[["Environment", payload.webhookHealth.environment.toUpperCase()], ["Event ID", payload.webhookHealth.latestProcessed?.eventId || "Not stored"], ["Event type", payload.webhookHealth.latestProcessed?.eventType || "Not stored"], ["Related object", payload.webhookHealth.latestProcessed?.relatedObjectId || "Not stored"], ["Idempotency", payload.webhookHealth.idempotency.evidence]]} /></div></details>
  </div>;
}
...
export function CustomerEmailsPage() {
  ...
  useEffect(() => { void load(); }, [load]);
  const choose = (key: string) => { setSelected(key); setDraft(payload?.templates.find((item) => item.templateKey === key) || null); setPreview(null); };
  ...
  const submit = async (event: FormEvent) => {
    ...
    const stop = startLoading("Saving structured email template"); setError("");
    try { const next = await saveCommerceTemplate(csrfToken, draft); setPayload(next); setDraft(next.templates.find((item) => item.templateKey === draft.templateKey) || draft); showToast("Draft template saved. No email was sent.", { title: "Email template saved" }); }
  ...
  const sendTest = async () => { if (!draft || !csrfToken || !recipient) return; setError(""); try { const result = await sendCommerceTemplateTest(csrfToken, draft.templateKey, recipient); showToast(result.duplicate ? "The deterministic TEST/PREVIEW delivery had already been sent; no duplicate was created." : `TEST/PREVIEW email sent to ${result.recipient}.`, { title: result.duplicate ? "Duplicate prevented" : "Test email sent", tone: result.duplicate ? "info" : "success" }); } catch (reason) { setError(errorMessage(reason, "The protected test email could not be sent.")); } };
  return <>
...
export function CommerceProductsPage() {
  ...
  const [reconciliation, setReconciliation] = useState<CatalogueReconciliationStatus | null>(null);
  const [reconciliationPreview, setReconciliationPreview] = useState<CatalogueReconciliationPreview | null>(null);
  ...
  useEffect(() => {
    ...
    if (!id) return;
    let active = true;
    void getMerchandisingProduct(id).then(result => { if (active) setSelectedProduct(result.product); }).catch(reason => { if (active) setError(errorMessage(reason, "Product editor unavailable.")); });
    return () => { active = false; };
  }, []);
  ...
  const [catalogue, setCatalogue] = useState<ProductListFilters["catalogue"]>("current");
  ...
  const mainFilters = useMemo<ProductListFilters>(() => ({ page, pageSize, query, visibility, status, migration, category, catalogue, sort }), [catalogue, category, migration, page, pageSize, query, sort, status, visibility]);
  const matchingFilters = useMemo<Omit<ProductListFilters, "page" | "pageSize">>(() => ({ query, visibility, status, migration, category, featured: "all", catalogue, sort }), [catalogue, category, migration, query, sort, status, visibility]);
  ...
  useEffect(() => { let active = true; void getCatalogueReconciliationStatus().then((next) => { if (active) setReconciliation(next); }).catch((reason) => { if (active) setError(errorMessage(reason, "Catalogue reconciliation status is unavailable.")); }); return () => { active = false; }; }, [refreshKey]);
  ...
  useEffect(() => { setSelectedIds([]); setAllMatching(false); }, [catalogue, category, migration, pageSize, query, sort, status, visibility]);
  ...
  const previewReconciliation = async () => {
    if (!csrfToken || !access.isMasterAdmin || reconciliationBusy) return;
    setReconciliationBusy(true); setError(""); setReconciliationPreview(null); setReconciliationConfirmation("");
    try { setReconciliationPreview(await previewCatalogueReconciliation(csrfToken)); }
    catch (reason) { setError(errorMessage(reason, "The read-only Printful reconciliation preview failed safely.")); }
    finally { setReconciliationBusy(false); }
  };
  const applyReconciliation = async () => {
    if (!csrfToken || !access.isMasterAdmin || !reconciliationPreview || reconciliationBusy) return;
    setReconciliationBusy(true); setError("");
    try {
      const result = await applyCatalogueReconciliation(csrfToken, reconciliationPreview.runId, reconciliationConfirmation);
      showToast(`Catalogue reconciliation applied: ${result.providerProducts} current products and ${result.providerVariants} current variants.`, { title: "Catalogue reconciled" });
      setReconciliationPreview(null); setReconciliationConfirmation(""); setCatalogue("current"); refresh();
    } catch (reason) { setError(errorMessage(reason, "Catalogue reconciliation was not applied.")); }
    finally { setReconciliationBusy(false); }
  };
  ...
  const saveFeatured = async () => {
    ...
    try {
      ...
      showToast("Featured order saved to Commerce D1.", { title: "Featured order saved" });
      refresh();
    } catch (reason) {
  ...
  const toggleFeatured = async (product: MerchandisingProduct) => {
    ...
    try {
      ...
      setFeaturedErrors((current) => ({ ...current, [product.id]: featuredErrorMessage(reason) }));
    } finally {
  ...
  const toggleVisibility = async (product: MerchandisingProduct) => {
    ...
    try {
      ...
      showToast((operation === "show" ? "Shown in store. " : "Hidden from store. ") + String(result.updated) + " product updated.", { title: "Storefront visibility updated" });
      refresh();
    } catch (reason) {
  ...
  const applyBulk = async (operation: ProductBulkOperation) => {
    ...
    try {
      ...
      showToast(bulkOperationLabel(operation) + " completed: " + String(result.updated) + " updated; " + String(result.unchanged) + " unchanged; " + String(result.rejected) + " rejected.", { title: "Bulk update complete" });
  ...
  return <>
    ...
    {!payload && !error ? <CommerceState>Loading catalogue merchandising…</CommerceState> : payload ? <div className="merchandising-workspace commerce-catalogue-manager">
      ...
      <section className="commerce-section" aria-labelledby="catalogue-products-title">
        ...
        {bulkMode && <div className="commerce-bulk-toolbar" aria-label="Bulk product actions"><strong aria-live="polite">{selectedCount} selected</strong><div><button type="button" className="text-button" onClick={() => { setAllMatching(false); setSelectedIds((current) => [...new Set([...current, ...payload.items.map((product) => product.id)])]); }}>Select current page</button><button type="button" className="text-button" onClick={() => { setSelectedIds([]); setAllMatching(false); }}>Clear selection</button><button type="button" className="text-button" onClick={() => { setSelectedIds([]); setAllMatching(true); }}>Select all {payload.totalItems} matching</button></div><CatalogueSellabilityReview scope={allMatching ? { kind: "matching", matching: matchingFilters } : { kind: "selected", productIds: selectedIds }} label={allMatching ? "Review matching catalogue fixes" : "Review selected catalogue fixes"} disabled={!selectedCount || busy} onApplied={refresh} /><CatalogueSellabilityReview label="Review all current catalogue fixes" disabled={busy} onApplied={refresh} /><div className="commerce-bulk-toolbar__actions"><button type="button" onClick={() => void applyBulk("show")} disabled={!selectedCount || busy}>Publish with eligible variants</button><button type="button" onClick={() => void applyBulk("hide")} disabled={!selectedCount || busy}>Hide from store</button><button type="button" onClick={() => void applyBulk("feature")} disabled={!selectedCount || busy}>Feature</button><button type="button" onClick={() => void applyBulk("unfeature")} disabled={!selectedCount || busy}>Unfeature</button></div>{allMatching && <small>All {payload.totalItems} products matching the current search and filters will be affected. You’ll confirm before applying.</small>}</div>}
        ...
        <div className="commerce-product-table" role="list">{payload.items.map((product) => {
          ...
          const providerEligible = !reconciliation?.counts.currentProducts || product.provider.presence === "current";
          const featuredEligibility = productFeaturedEligibility(product, Boolean(reconciliation?.counts.currentProducts));
          const presentation = productOperationalPresentation(product);
      ...
      {selectedProduct && <ProductMerchandisingEditor product={selectedProduct} collections={collections} csrfToken={csrfToken} canManage={canManage} onClose={() => setSelectedProduct(null)} onSaved={(product, notice) => { replaceProduct(product); showToast(notice, { title: "Merchandising saved" }); }} onError={setError} />}
...
export function CommerceCollectionsPage() {
  ...
  const refresh = (notice: string) => { if (notice) showToast(notice, { title: "Collections updated" }); setRefreshKey((value) => value + 1); };
...
function CopyValueButton({ label, value }: { label: string; value: string }) { const { showToast } = useAdminToast(); return <button type="button" className="secondary-button payments-copy-button" onClick={() => { void navigator.clipboard.writeText(value).then(() => showToast("The value is ready to paste.", { tone: "info", title: "Copied to clipboard" })).catch(() => showToast("Clipboard access was unavailable. Try copying the value manually.", { tone: "warning", title: "Copy unavailable" })); }}>{label}</button>; }
...
type ProductStatusTone = "healthy" | "warning" | "error" | "muted";
...
function productOperationalPresentation(product: MerchandisingProduct): { storefront: ProductStatusPresentation; fulfillment: ProductStatusPresentation; migration: ProductStatusPresentation } {
  ...
  else if (["restricted", "error"].includes(product.status)) storefront = { label: "Blocked", tone: "error" };
  else storefront = product.visibility === "public" && product.status === "active" ? { label: "Public", tone: "healthy" } : { label: "Hidden", tone: "warning" };

  let fulfillment: ProductStatusPresentation;
  if (product.provider.presence === "provider_missing") fulfillment = { label: "Provider missing", tone: "error" };
  else if (product.provider.presence === "wrong_store") fulfillment = { label: "Wrong store", tone: "error" };
  else if (product.provider.archivedAt || product.provider.reconciliationStatus === "archived") fulfillment = { label: "Archived", tone: "muted" };
  else if (["needs_review", "ambiguous"].includes(product.provider.reconciliationStatus)) fulfillment = { label: "Needs review", tone: "warning" };
  else if (product.readiness.fulfillment === "mapped") fulfillment = { label: "Mapped", tone: "healthy" };
  else if (product.readiness.fulfillment === "blocked") fulfillment = { label: "Blocked", tone: "error" };
  else if (product.readiness.fulfillment === "pending") fulfillment = { label: "Pending", tone: "warning" };
  else fulfillment = { label: humanize(product.readiness.fulfillment || "not configured"), tone: "muted" };

  const migrationValue = product.migrationStatus || "not configured";
  const migration: ProductStatusPresentation = ["target_verified", "current"].includes(migrationValue) ? { label: humanize(migrationValue), tone: "healthy" }
    : ["blocked", "error", "conflict"].includes(migrationValue) ? { label: humanize(migrationValue), tone: "error" }
      : ["legacy", "archived"].includes(migrationValue) ? { label: humanize(migrationValue), tone: "muted" }
        : { label: humanize(migrationValue), tone: "warning" };
  return { storefront, fulfillment, migration };
}
function statusToneClass(tone: ProductStatusTone) { return `commerce-operational-status is-${tone}`; }
function productFeaturedEligibility(product: MerchandisingProduct, hasCurrentAuthority: boolean): { enabled: boolean; reason: string } {
  const reconciledAuthority = hasCurrentAuthority || product.provider.presence !== "legacy" || product.provider.reconciliationStatus !== "legacy";
  if (!reconciledAuthority) return ["active", "legacy_production"].includes(product.status) ? { enabled: true, reason: "" } : { enabled: false, reason: "Only active legacy products can be Featured before provider reconciliation" };
  if (product.provider.presence === "provider_missing") return { enabled: false, reason: "Provider-missing products cannot be Featured" };
  if (product.provider.presence === "wrong_store") return { enabled: false, reason: "Wrong-store products cannot be Featured" };
  if (product.provider.archivedAt || product.provider.reconciliationStatus === "archived") return { enabled: false, reason: "Archived products cannot be Featured" };
  if (product.provider.reconciliationStatus === "ambiguous") return { enabled: false, reason: "Products with ambiguous provider identity cannot be Featured" };
  if (product.provider.presence !== "current" || product.provider.reconciliationStatus !== "current") return { enabled: false, reason: "This product requires provider review before it can be Featured" };
  return { enabled: true, reason: "" };
}
function ProductThumbnail({ product }: { product: MerchandisingProduct }) {
...
function featuredErrorMessage(reason: unknown) { const message = errorMessage(reason, ""); return !message || /account service is unavailable|failed to fetch/i.test(message) ? "Could not update Featured status. Try again." : message; }
...
function productionActivationLabel(state: PaymentsControlPlanePayload["productionActivation"]["state"]) { return ({ active: "Active", action_required: "Action required", disabled: "Disabled", paused: "Emergency paused" } as const)[state]; }
function productionActivationCopy(state: PaymentsControlPlanePayload["productionActivation"]["state"]) { return ({ active: "Production commerce is enabled through the canonical launch authority. This card is a read-only projection of the active gates.", action_required: "Production commerce needs attention. Activation remains governed by the canonical launch workflow rather than this status card.", disabled: "Production commerce remains disabled. Activation is governed by the canonical launch workflow rather than this status card.", paused: "Production commerce is emergency-paused. This read-only card shows the underlying canonical authority." } as const)[state]; }
function paymentStateLabel(state: PaymentAuthorityState | PaymentGateState) { return ({ configured: "Configured", verified: "Verified", unverified: "Unverified", disabled: "Disabled", unavailable: "Unavailable", active: "Active", paused: "Emergency paused", closed: "Closed", ready: "Ready", action_required: "Action required", not_applicable: "Not applicable" } as const)[state]; }
