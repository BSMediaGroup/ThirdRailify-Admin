...
function PaymentsControlPlane({ payload }: { payload: PaymentsControlPlanePayload }) {
  ...
  return <div className="payments-workspace">
    ...
    <section className="commerce-section" aria-labelledby="paypal-provider-title"><SectionTitle id="paypal-provider-title" eyebrow="Preferred processor" title="PayPal direct merchant" /><div className="payments-primary-grid">
      <article className="payments-panel payments-provider-card has-provider-feature"><img className="provider-feature-icon" src={paypalFeatureIcon} alt="" aria-hidden="true" /><header><div><p className="eyebrow">Preferred · Orders API v2</p><h3>PayPal</h3></div><PaymentStateChip state={payload.paypal.credentialConfigured ? "configured" : "unverified"} label={payload.paypal.credentialConfigured ? "Configured" : "Setup required"} /></header><p>One direct-merchant CAPTURE integration serves physical store orders and one-time donations through separate local authorities.</p><dl><Fact term="Sandbox Client ID" value={payload.paypal.sandbox.clientIdConfigured ? "Configured" : "Not configured"} /><Fact term="Sandbox Client Secret" value={payload.paypal.sandbox.clientSecretConfigured ? "Configured server-side" : "Not configured"} /><Fact term="Sandbox Webhook ID" value={payload.paypal.sandbox.webhookIdConfigured ? "Configured / masked" : "Not configured"} /><Fact term="LIVE Client ID" value={payload.paypal.live.clientIdConfigured ? "Configured" : "Not configured"} /><Fact term="LIVE Client Secret" value={payload.paypal.live.clientSecretConfigured ? "Configured server-side" : "Not configured"} /><Fact term="LIVE Webhook ID" value={payload.paypal.live.webhookIdConfigured ? "Configured / masked" : "Not configured"} /><Fact term="Store checkout" value={payload.paypal.shopCheckoutEnabled ? "Enabled" : "Disabled"} /><Fact term="Donations" value={payload.paypal.donationsEnabled ? "Enabled" : "Disabled"} /><Fact term="Latest verified delivery" value={formatSynchronizedAt(payload.paypal.latestVerifiedDelivery?.received_at || null)} /></dl><details className="payments-technical"><summary>Webhook event set</summary><p>{payload.paypal.webhookEvents.join(" · ")}</p><p className="commerce-action-note">This read-only page makes no PayPal API call and never projects credentials.</p></details></article>
