...
function PaymentsControlPlane({ payload }: { payload: PaymentsControlPlanePayload }) {
  ...
  return <div className="payments-workspace">
    ...
    <section className="commerce-section" aria-labelledby="paypal-provider-title"><SectionTitle id="paypal-provider-title" eyebrow="Preferred processor" title="PayPal direct merchant" /><div className="payments-primary-grid">
      ...
      <article className="payments-panel payments-provider-card has-provider-feature"><img className="provider-feature-icon" src={stripeFeatureIcon} alt="" aria-hidden="true" /><header><div><p className="eyebrow">Future card-processing option</p><h3>{stripe.displayName}</h3></div><PaymentStateChip state="disabled" label="CONFIGURED / DISABLED" /></header><p>Stripe remains fully implemented and its historical TEST evidence is preserved. It is non-preferred and excluded from current PayPal activation readiness.</p><dl><Fact term="Integration" value="Direct merchant" /><Fact term="Current use" value="Disabled" /><Fact term="New Checkout Sessions" value="Prohibited" /><Fact term="TEST acceptance" value={evidence ? "Preserved" : "Unverified"} /><Fact term="Future restoration" value="Requires a separately authorized milestone" /></dl></article>
