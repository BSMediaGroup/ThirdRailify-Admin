...
export function CommerceOverviewPage() {
  ...
  return <>
    <CommerceHeading eyebrow="Admin-only control plane" title="Commerce overview" summary="The dedicated Third Railify Official Canadian Stripe test API is connected and the webhook endpoint is ready for signing-secret configuration. Checkout, live payment, and fulfillment actions remain disabled." status="disabled" />
...
export function PaymentsPayoutsPage() {
  ...
  return <>
    <CommerceHeading eyebrow="Processor ownership" title="Payments & payouts" summary={connected ? "The dedicated Canadian Stripe merchant account is connected to the test API. The webhook endpoint is ready for signing-secret configuration; Checkout, live payments, and live payout readiness remain disabled or unverified." : "The dedicated Third Railify Official Stripe account exists. Test API verification is available only through the protected server action; the webhook endpoint is ready but signing, Checkout Sessions, live activation, and payout readiness remain unavailable."} status={connected ? "connected" : "setup_required"} statusLabel={connected ? "Test API connected" : undefined} />
    {error && <div className="admin-alert" role="alert">{error}</div>}
