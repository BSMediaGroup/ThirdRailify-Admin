...
function CustomerDrawer({ customer, loading, close }: { customer: CustomerDetail | null; loading: boolean; close: () => void }) {
  ...
  return <>
    ...
    <div className="detail-drawer__body">
      ...
      <DrawerSection title="Order history">{customer.orders.length ? <div className="customer-order-history">{customer.orders.map((order) => <article key={order.id}><div><span className={`order-environment order-environment--${order.environment}`}>{order.environment.toUpperCase()}</span><strong>{compactId(order.id)}</strong><small>{dateTime(order.createdAt)}</small></div><div><strong>{money(order.totalAmount)}</strong><span>{human(order.paymentStatus)} · {human(order.fulfillmentStatus)}</span></div><div><span>{order.delivery ? `${[order.delivery.regionCode, order.delivery.countryCode].filter(Boolean).join(" / ")} · historical delivery snapshot` : "No delivery snapshot"}</span><small>{order.documentCount} documents · {order.emailCount} email events</small></div><Link to={`/orders?order=${encodeURIComponent(order.id)}`}>View order</Link></article>)}</div> : <State>No linked orders.</State>}</DrawerSection>
