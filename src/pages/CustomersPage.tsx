...
function CustomerRow({ customer, open }: { customer: CommerceCustomer; open: (id: string) => void }) {
  ...
  return <tr tabIndex={0} aria-label={`Open customer ${customer.contact.name}`} onClick={activate} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); activate(); } }}>
    ...
    <Cell label="Orders"><strong>{customer.summary.orderCount}</strong><span>{customer.summary.paidOrderCount} paid · {customer.summary.donationCount ?? 0} donations</span></Cell>
...
function CustomerDrawer({ customer, loading, close }: { customer: CustomerDetail | null; loading: boolean; close: () => void }) {
  ...
  return <>
    ...
    <div className="detail-drawer__body">
      ...
      <DrawerSection title="Commerce summary"><div className="customer-summary-grid"><Metric label="Orders" value={customer.summary.orderCount} /><Metric label="Paid" value={customer.summary.paidOrderCount} /><Metric label="Donations" value={customer.summary.donationCount ?? 0} /><Metric label="Completed donations" value={customer.summary.completedDonationCount ?? 0} /><Metric label="LIVE order gross" value={money(customer.summary.liveSpendAmount)} /><Metric label="LIVE donations" value={money(customer.summary.liveDonationAmount ?? 0)} /><Metric label="TEST order gross" value={money(customer.summary.testSpendAmount)} /><Metric label="SANDBOX donations" value={money(customer.summary.sandboxDonationAmount ?? 0)} /></div><p className="order-readonly-note">TEST and SANDBOX values are evidence only and are never included in LIVE customer value.</p></DrawerSection>
