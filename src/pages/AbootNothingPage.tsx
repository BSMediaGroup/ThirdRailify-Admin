import { DeletePollButton } from '../polls/DeletePollButton';
...
const empty = { streamUrl: '', livestreamMode: 'automatic', livestreamId: '', title: '', description: '', rumbleEnabled: false, rumbleSourceScope: '', presentationType: 'abootnothing', presentation: { colors: ['#f3c928', '#a9b7da'], context: '', featuredOrder: 0 }, options: [{ label: 'Side one', trigger: '1', description: '' }, { label: 'Side two', trigger: '2', description: '' }] };
export function AbootNothingPage() {
  ...
  const edit = (poll: Matchup | null) => { setPendingArtwork({}); setSelected(poll); setEditing(true); setDraft(poll ? { ...empty, ...poll, streamUrl: poll.streamUrl || '', description: poll.description || '', rumbleSourceScope: poll.rumbleSourceScope || '', presentationType: 'abootnothing', presentation: { colors: poll.presentation?.colors || empty.presentation.colors, context: poll.presentation?.context || '', featuredOrder: poll.presentation?.featuredOrder || 0 }, options: poll.options.map(o => ({ ...o, description: o.description || '' })) } : structuredClone(empty)); };
  useEffect(() => { if (!editSlug) return; void pollAdminRequest<{ poll: Matchup }>("/api/admin/polls/" + encodeURIComponent(editSlug)).then(p => { setSelected(p.poll); setEditing(true); setDraft({ ...empty, ...p.poll, streamUrl: p.poll.streamUrl || '', description: p.poll.description || '', rumbleSourceScope: p.poll.rumbleSourceScope || '', presentationType: 'abootnothing', presentation: { colors: p.poll.presentation?.colors || empty.presentation.colors, context: p.poll.presentation?.context || '', featuredOrder: p.poll.presentation?.featuredOrder || 0 }, options: p.poll.options.map(o => ({ ...o, description: o.description || '' })) }); }).catch(e => setError(e.message)); }, [editSlug]);
  ...
  return <div className="poll-admin aboot-admin"><header className="poll-heading"><div><p className="eyebrow">THE THIRD RAIL · CONTENT & OPERATIONS</p><h1>Aboot Nothing</h1><Link to="/polls/abootnothing/brackets">Matchup Studio</Link>{returnBracket ? <Link to={`/polls/abootnothing/brackets/${encodeURIComponent(returnBracket)}`}>Return to bracket</Link> : null}<p>Two opposing subjects. One Poll, shared votes and a permanent matchup history.</p></div><div><button className="primary-button" disabled={!canManage || busy} onClick={() => edit(null)}>Create matchup</button><Link to="/automations#poll-voting">Voting policy & reconciliation</Link></div></header>{error && !editing ? <div className="poll-workspace-alert" role="alert"><strong>Unable to save matchup</strong><p>{error}</p></div> : null}
    <section className="poll-panel aboot-library" aria-busy={loading}><header><div><h2>Matchup library</h2><p>Create, manage and revisit your matchups.</p></div><label>Matchups<select value={state} onChange={e => setState(e.target.value)}><option value="all">All matchups</option><option value="open">Current</option><option value="closed">Past</option><option value="draft">Drafts</option><option value="archived">Archived</option></select></label></header><div className="poll-workspace-body">{loadError ? <div className="poll-workspace-alert" role="alert"><strong>Unable to load matchups</strong><p>{loadError}</p><button type="button" onClick={() => void load()}>Try again</button></div> : null}<div className="aboot-admin-grid">{items.map(p => <article key={p.id} className={p.state === "archived" ? "is-archived" : undefined}><div className="aboot-admin-versus">{p.options.map((o, i) => <div key={o.id} style={{ borderColor: p.presentation?.colors?.[i] }} >{o.image ? <img src={`/api/admin/polls/media/${o.image.id}`} alt="" /> : null}<strong>{o.label}</strong></div>)}<b>VS</b></div><h2>{p.title}</h2><p>{p.state === "draft" && !p.openedAt ? "Upcoming" : p.state} · {p.public ? 'Listed' : 'Hidden'} · {p.totalVotes} votes · {p.credits?.unresolved ?? 'Unknown'} awaiting allocation</p><div className="poll-actions"><DeletePollButton poll={p} onDeleted={() => void load()} />{p.openedAt ? <button disabled={!canManage || busy} onClick={() => { if (window.confirm('Reset to Upcoming? Current results will be retained in history, votes will start at zero, and unallocated credits will be discarded.')) void action(() => mutatePollLifecycle(csrfToken || '', p, 'reset'), 'Matchup reset to Upcoming'); }}>Reset to Upcoming</button> : null}<button disabled={!canManage || busy} onClick={() => edit(p)}>Edit Poll</button>{p.state === 'open' ? <button disabled={!canManage || busy} onClick={() => void action(() => mutatePollLifecycle(csrfToken || '', p, 'close'))}>Close</button> : p.state !== 'archived' ? <button disabled={!canManage || busy} onClick={() => void action(() => mutatePollLifecycle(csrfToken || '', p, 'open'))}>Open</button> : null}{p.state === 'closed' || (p.state === 'draft' && !p.openedAt) ? <button disabled={!canManage || busy} onClick={() => void action(() => mutatePollVisibility(csrfToken || '', p, !p.public), p.public ? 'Poll hidden' : 'Poll published')}>{p.public ? 'Hide from gallery' : p.state === 'draft' ? 'Publish Upcoming' : 'Publish results'}</button> : null}{['draft', 'closed'].includes(p.state) ? <button disabled={!canManage || busy} onClick={() => void action(() => mutatePollLifecycle(csrfToken || '', p, 'archive'))}>Archive</button> : null}{p.state === 'archived' ? <button disabled={!canManage || busy} onClick={() => void action(() => mutatePollLifecycle(csrfToken || '', p, 'restore'))}>Restore draft</button> : null}<a href={`https://thirdrailify.com/polls/${p.slug}`} target="_blank" rel="noopener noreferrer">Public detail</a></div></article>)}</div>{!items.length && !loadError ? <div className="poll-workspace-empty"><span className="aboot-empty-mark" aria-hidden="true">VS</span><strong>{loading ? 'Loading matchups...' : 'No matchups in this view'}</strong><p>{loading ? 'Fetching your matchup library.' : 'Start with two opposing subjects, then add artwork and open the vote when you are ready.'}</p>{!loading ? <button type="button" className="primary-button" disabled={!canManage || busy} onClick={() => edit(null)}>Create your first matchup</button> : null}</div> : null}</div></section>
    {editing ? <dialog ref={dialogRef} className="aboot-editor-dialog" aria-modal="true" aria-labelledby="aboot-editor-title" data-admin-toast-host onCancel={event => { event.preventDefault(); if (!busy) closeEditor(); }}><form ref={editorRef} className="poll-panel aboot-matchup-editor" onSubmit={save}><header><h2 id="aboot-editor-title">{selected ? 'Edit matchup' : 'New matchup'}</h2>{selected?.bracketLink ? <Link to={`/polls/abootnothing/brackets/${selected.bracketLink.bracketId}`}>Bracket: {selected.bracketLink.title} ? Round {selected.bracketLink.round + 1}, match {selected.bracketLink.position + 1}</Link> : null}<button type="button" disabled={busy} onClick={closeEditor}>Close editor</button></header><div className="poll-workspace-body">{error ? <div className="poll-workspace-alert" role="alert"><strong>Unable to save matchup</strong><p>{error}</p></div> : null}<div className="poll-policy-fields"><AbootPollFields title={draft.title} options={draft.options} locked={locked} onTitle={title => setDraft({ ...draft, title })} onTrigger={(index, trigger) => setDraft({ ...draft, options: draft.options.map((o,i) => i === index ? { ...o, trigger } : o) })} /><PollStreamField value={draft.streamUrl || ''} onChange={streamUrl => setDraft({ ...draft, streamUrl })} slug={selected?.slug} disabled={busy || !canManage} /><label>Context / episode<input maxLength={160} value={draft.presentation.context} onChange={e => setDraft({ ...draft, presentation: { ...draft.presentation, context: e.target.value } })} /></label><label>Description<textarea maxLength={2000} value={draft.description} onChange={e => setDraft({ ...draft, description: e.target.value })} /></label>
