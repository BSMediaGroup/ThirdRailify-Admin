...
type Payload = { strategy?: string; policy: { active_ratebook_id: string | null; revision: number }; books: { id: string; revision: number; status: string; body: Book }[]; markets: { country_code: string; display_name: string }[]; coverage: { total: number; covered: number; missing: { productId: string; productName: string; variantId: string; sku: string }[] } };
...
export function ShippingRatesWorkspace() {
  ...
  return <section className="shipping-rates" id="shipping-rates" aria-labelledby="shipping-rates-title">
    ...
    {!book || !payload ? <p>Loading shipping ratebook…</p> : <>
      ...
      {payload.strategy === "printful_dynamic" && <p><strong>Printful Dynamic · Active</strong> — Local weights are not required. This weight-band ratebook is not the active shipping strategy.</p>}
      <details><summary>Merchant ratebook weight coverage: {payload.coverage.covered} / {payload.coverage.total} current checkout-eligible variants</summary><p>Includes the variants’ configured checkout environments. Weights are required only to publish merchant weight-band pricing; they do not block Printful Dynamic.</p><ul>{payload.coverage.missing.map(v => <li key={v.variantId}><a href="/products">{v.productName}</a> · {v.variantId} · {v.sku || "No SKU"}</li>)}</ul></details>
    </>}
  </section>;
}
