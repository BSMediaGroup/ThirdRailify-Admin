...
export type ProductionReadiness = { ok: boolean; authority: string; phase: "preflight" | "active" | "paused"; productionReady: boolean; operationalReady?: boolean; operationalState?: ProductionOperationalState; mandatoryDomains: string[]; domains: Record<string, ReadinessDomain>; checkedAt: string };
...
export type ProductionOperationalState = "preflight" | "active" | "paused" | "degraded";
export type ProductionActivationState = "active" | "action_required" | "disabled" | "paused";
export type PaymentAuthorityState = "configured" | "verified" | "unverified" | "disabled" | "unavailable" | "active" | "paused" | "closed";
...
export type PaymentsControlPlanePayload = {
  ...
  paypal: { provider: "paypal"; state: "disabled" | "deferred" | "setup_required" | "connected"; integrationMode: "direct_merchant" | "unavailable"; environment: string; countryCode: string | null; currencyCode: string | null; credentialConfigured: boolean; preferred: boolean; sandbox: PayPalEnvironmentState; live: PayPalEnvironmentState; donationsEnabled: boolean; membershipEnabled: boolean; shopCheckoutEnabled: boolean; captureEnabled: boolean; storeCaptureEnabled: boolean; donationCaptureEnabled: boolean; webhookEvents: string[]; webhookUrl?: string | null; setupCommand?: string; latestVerifiedDelivery: null | { environment: string; event_type: string; received_at: string; result_code: string }; attempts: Array<{ environment: string; kind?: "store" | "donation"; normalized_state: string; count: number }>; donations: Array<{ environment: string; status: string; count: number; amount: number }>; diagnostics: Array<{ operation_kind: string; http_status: number | null; provider_code: string | null; provider_reason: string | null; request_id: string | null; retryable: number; occurred_at: string }>; providerMutationAvailable: false; lastVerifiedAt: string | null };
  gates: PaymentGate[];
  productionActivation: { state: ProductionActivationState; operationalState: ProductionOperationalState; emergencyPaused: boolean; checkout: { enabled: boolean; state: PaymentAuthorityState }; livePayments: { enabled: boolean; state: PaymentAuthorityState }; fulfillment: { enabled: boolean; state: PaymentAuthorityState }; controlledTestCheckout: { enabled: false; state: "closed"; purpose: "controlled_test"; acceptanceEvidence: "preserved" | "unverified" }; mutableFromThisRoute: false };
...
export type FulfillmentShippingPayload = {
  webhook?: { subscription: string; custody: string; verifier: string; signedDelivery: string; lastEvent: { event_type: string; occurred_at: string } | null; url: string; events: string[] };
  ok: boolean; databaseConfigured: boolean; authority: string; access: CommerceAccess;
...
export type CommerceOrderDetail = {
  ...
  delivery: { pricingPolicy?: { ratebookId: string; revision: number; zoneId: string; methodId: string; weightMg: number; providerQuotedCost: number } | null; available: boolean; recipientConfigured: boolean; destinationCountryCode: string | null; destinationRegionCode: string | null; strategy: string | null; provider: string | null; method: string | null; amount: number | null; currencyCode: string | null; quoteReference: string | null; quotedAt: string | null; capturedAt: string | null; addressExternallyVerified: false };
