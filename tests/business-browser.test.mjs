...
test("Business Information is responsive, accessible, validation-safe, and supports explicit save/discard", async (t) => {
  ...
  for (const [width, height] of [[1440, 1000], [768, 1024], [390, 844]]) {
    ...
    await page.route("**/api/**", async (route) => {
      ...
      if (path === "/api/admin/commerce/business" && route.request().method() === "POST") { const body = JSON.parse(route.request().postData() || "{}"); savedBodies.push(body); payload = fixture({ supportEmail: body.supportEmail, businessPhone: body.businessPhone, businessAddress: body.businessAddress, revision: payload.profile.revision + 1 }); return json(route, payload); }
...
function fixture(overrides = {}) {
  ...
  return { ok: true, databaseConfigured: true, encryptionConfigured: true, access: { isMasterAdmin: true, capabilities: ["commerce.view", "commerce.business.manage"] }, authority: "Commerce D1", privacy: { publicSafe: ["trading_name"], adminOnly: ["business_address", "business_phone", "revision"], sensitive: ["legal_business_name", "legal_business_address"] }, profile: { tradingName: "Third Railify Official", countryCode: "CA", provinceCode: "ON", currencyCode: "CAD", businessAddress, publicAddress: businessAddress, publicContactEmail: "info@thirdrailify.com", supportEmail, businessPhone, publicPhone: businessPhone, websiteUrl: "", invoicePrefix: "", documentFooter: "", taxProviderState: "unavailable", invoiceAccentColor: "#f3c928", receiptAccentColor: "#f3c928", revision, updatedAt: "2026-08-29T00:00:00Z", private: { legalBusinessNameStored: false, privateAddressStored: false, privatePhoneStored: false, businessRegistrationNumberStored: false, legalBusinessNameMasked: "", privateAddressMasked: "", privatePhoneMasked: "", businessRegistrationNumberMasked: "", registrations: [] } }, readiness: { overallStatus: "action_required", completion: { complete: completeContact ? 6 : 5, total: 9, percent: completeContact ? 67 : 56 }, groups, profile: { coreIdentity: "complete", publicContact: completeContact ? "complete" : "partial", legalIdentity: "not_configured", address: Object.values(businessAddress).some(Boolean) ? "partial" : "not_configured", tax: "not_configured", documents: "partial", productionCommerce: "action_required" }, dependencies: { ...domains, paypalRequired: false }, documentIdentity: { tradingName: "Third Railify Official", legalNameStored: false, addressStored: Object.values(businessAddress).some(Boolean), contactEmail: supportEmail || "info@thirdrailify.com", taxRegistrationState: "not_configured", receiptTemplate: { state: "complete", revision: 1 }, invoiceTemplate: { state: "incomplete", revision: 1 } } }, canonicalReadiness };
}
