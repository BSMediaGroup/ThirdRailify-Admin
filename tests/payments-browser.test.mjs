...
test("Payments and Payouts renders truthful responsive TEST-only control plane", async (t) => {
  ...
  for (const [width, height] of [[1920, 1080], [1440, 900], [768, 1024], [390, 844]]) {
    ...
    assert.equal(await paypalCard.getByText("Setup required", { exact: true }).count(), 1);
...
function payments() {
  ...
  return {
    ...
    paymentMethods: [{ id: "paypal", label: "PayPal", state: "unverified", detail: "Preferred standard PayPal experience; configuration is required." }, { id: "card", label: "Card payments", state: "disabled", detail: "Stripe is retained for a future activation." }],
