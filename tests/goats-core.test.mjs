...
test("guest draft finalisation stays private until validated approval, then projects no private fields", async (t) => {
  ...
  let sends = 0;
  const sent = await retryEmail({ ...env, RESEND_API_KEY: "test-key", MAIL_FROM: "Test <test@example.test>" }, outbox.id, "master-admin", async (_url, init) => { sends += 1; const body = JSON.parse(init.body); assert.equal(body.to[0], "private@example.test"); assert.equal(body.subject.includes(finalised.reference), true); assert.equal(body.html.includes("<script>"), false); assert.equal(body.html.includes("&lt;script&gt;"), true); return Response.json({ id: "email-test" }); });
