...
test('connected Studio, historical sample, media, publication privacy and responsive public roadmap', { timeout:180000 }, async t => {
  ...
  await changePollLifecycle(env,account.id,linked.public_slug,{action:'open',revision:live.revision});await page.reload();await page.locator('.bracket-round').first().locator('.bracket-match').last().dblclick();await page.getByText('Needs review. An accepted source or upstream result changed.',{exact:true}).waitFor();
