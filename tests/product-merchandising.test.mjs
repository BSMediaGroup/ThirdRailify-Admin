...
test("current Hidden and zero-public-variant products can store Featured while non-current products fail explicitly", async (t) => {
  ...
  await insertTestVariant(harness.commerceDb, { id: "current-public-variant", productId: "current-public", targetPrintfulProductId: "460339030", targetPrintfulSyncVariantId: "7001", migrationStatus: "target_verified" });
...
test("Full Admin catalogue parity permits edits while a Regular account is rejected", async (t) => {
  ...
  await harness.authDb.prepare("INSERT INTO accounts (id,email_normalized,display_name,role,admin_level,status,email_verified_at,created_at,updated_at,source) VALUES ('merch-full','full@example.test','Full Admin','admin','full','active',?,?,?,'test'),('merch-user','user@example.test','Regular User','user','none','active',?,?,?,'test')").bind(now, now, now, now, now, now).run();
  const fullAccount = await loadAccountByEmail(env, "full@example.test"); const fullSession = await createSession(env, new Request(`${ADMIN_ORIGIN}/`, { headers: { Origin: ADMIN_ORIGIN } }), fullAccount, ADMIN_ORIGIN); const fullCookie = cookiePair(fullSession.cookie); const url = `${ADMIN_ORIGIN}/api/admin/commerce/products/product-test-001`;
  const accepted = await commerceRequest({ request: jsonRequest(url, { origin: ADMIN_ORIGIN, cookie: fullCookie, csrfToken: fullSession.csrfToken, body: productBody() }), env, data: {} }); assert.equal(accepted.status, 200);
  const userAccount = await loadAccountByEmail(env, "user@example.test"); const userSession = await createSession(env, new Request(`${ADMIN_ORIGIN}/`, { headers: { Origin: ADMIN_ORIGIN } }), userAccount, ADMIN_ORIGIN); const userCookie = cookiePair(userSession.cookie);
  const rejected = await commerceRequest({ request: jsonRequest(url, { origin: ADMIN_ORIGIN, cookie: userCookie, csrfToken: userSession.csrfToken, body: productBody() }), env, data: {} }); assert.equal(rejected.status, 403);
});
...
test("public catalogue exposes only safe public D1 fields with exact variant prices and cache behavior", async (t) => {
  ...
  const response = await catalogueRequest({ env }); assert.equal(response.status, 200); assert.equal(response.headers.get("cache-control"), "no-store"); const payload = await response.json(); assert.equal(payload.products.length, 1); assert.equal(payload.products[0].variants[0].unitAmount, 2750); assert.equal(payload.products[0].price.minUnitAmount, 2750); assert.equal(payload.products[0].price.maxUnitAmount, 2750); assert.deepEqual(payload.products[0].categories, ["Apparel"]); assert.deepEqual(payload.products[0].collectionSlugs, ["apparel"]); assert.equal(payload.collections.some((collection) => collection.slug === "third-rail-lore"), false); assert.equal(payload.collections.find((collection) => collection.slug === "apparel").productCount, 1); assert.deepEqual(payload.collections.map((collection) => collection.displayOrder), [...payload.collections.map((collection) => collection.displayOrder)].sort((a, b) => a - b)); assert.doesNotMatch(JSON.stringify(payload), /printful|legacy|migration|sku|safe_metadata|source_provider|archived/i);
