...
test("Stage authority enforces ownership, independent Wheel access, visibility, revisions, and max six", async (t) => {
  ...
  const creatorHiddenWheel = await createWheel(env, "creator", wheelInput("Private Alpha", "hidden"));
  const hiddenWheel = await createWheel(env, "other", wheelInput("Private Beta", "hidden"));
  await assert.rejects(createStage(env, "creator", { title: "Private Stage", visibility: "private", wheelSlugs: [publicWheel.wheel.slug, hiddenWheel.wheel.slug] }), (error) => error.code === "stage_wheel_owner_mismatch");
  ...
  await assert.rejects(createStage(env, "creator", { title: "Assigned Is Not Owned", visibility: "private", wheelSlugs: [hiddenWheel.wheel.slug] }), (error) => error.code === "stage_wheel_owner_mismatch");
  const privateStage = await createStage(env, "creator", { title: "Private Stage", visibility: "private", wheelSlugs: [publicWheel.wheel.slug, creatorHiddenWheel.wheel.slug] });
  ...
  await assert.rejects(createStage(env, "creator", { title: "Leaky Stage", visibility: "public", wheelSlugs: [creatorHiddenWheel.wheel.slug] }), (error) => error.code === "public_stage_private_wheel");
  ...
  const publicDirectory = await listPublicStages(env); assert.equal(publicDirectory.items.length, 1); assert.deepEqual(publicDirectory.items[0].owner, { displayName: "Stage Creator", avatarUrl: null });
  ...
  const lookup = await listAccessibleWheels(env, "creator"); assert.ok(lookup.items.some((item) => item.slug === creatorHiddenWheel.wheel.slug && item.capability === "Edit")); assert.equal(lookup.items.some((item) => item.slug === hiddenWheel.wheel.slug), false);
  await mutateWheelAssignment(env, "master", { wheelId: hiddenId, accountId: "creator", role: "spinner", action: "revoke" });
  await changeWheelLifecycle(env, "creator", creatorHiddenWheel.wheel.slug, { action: "archive" });
  const afterArchive = await getStage(env, privateStage.stage.slug, "creator"); assert.equal(afterArchive.stage.wheels[1].wheel.lifecycle, "archived");
  ...
  assert.equal(Number((await harness.commerceDb.prepare("SELECT COUNT(*) AS count FROM wheels").first()).count), 3);
});
