  ...
  for (const width of [1920, 768, 390, 1440]) {
    ...
    if (width === 1440) {
      ...
      for (const toastWidth of [1440, 390]) {
        ...
        const toastFit = await page.locator('.admin-toast-region').evaluate(n => { const r = n.getBoundingClientRect(); return getComputedStyle(n).position === 'fixed' && r.left >= 0 && r.right <= innerWidth && r.top >= 0 && r.bottom <= innerHeight; });
        assert.ok(toastFit);
    ...
    if (width === 1440) {
      ...
      await page.getByLabel('Title', { exact: true }).fill('Artwork before first save');
      ...
      assert.equal(await count(), beforeCount + 1);
      await page.getByLabel('Title', { exact: true }).fill('Unsaved title survives artwork replacement');
      await page.getByLabel('Square artwork').first().setInputFiles(file);
      await page.waitForFunction(() => !document.querySelector('input[type=file]').disabled);
      assert.equal(await page.getByLabel('Title', { exact: true }).inputValue(), 'Unsaved title survives artwork replacement');
      await page.reload();
