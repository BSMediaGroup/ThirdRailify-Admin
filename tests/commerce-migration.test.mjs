...
test("commerce migrations apply in order, with the idempotent foundations repeat-safe", async (t) => {
  ...
  assert.deepEqual(result.results.map((row) => row.name), [
    ...
    "commerce_audit", "commerce_business_profiles", "commerce_catalogue_migrations", "commerce_collections", "commerce_customers", "commerce_email_deliveries", "commerce_fulfillment_order_items", "commerce_fulfillment_orders", "commerce_fulfillment_shipment_items", "commerce_fulfillment_shipments", "commerce_order_delivery_snapshots", "commerce_order_documents", "commerce_order_items", "commerce_orders", "commerce_permission_grants", "commerce_printful_file_mappings", "commerce_product_collections", "commerce_product_variants", "commerce_products",
    "commerce_provider_connections", "commerce_provider_webhook_events", "commerce_saved_addresses", "commerce_settings", "commerce_shipping_quotes", "commerce_tax_registrations", "commerce_templates", "commerce_webhook_events",
