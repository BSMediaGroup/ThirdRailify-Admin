...
test('Studio canvas fullscreen, measured connectors, focus, modal editors, bench and card navigation', { timeout:180000 }, async t => {
  ...
  await context.route('**/*',async route=>{
    ...
    else if(u.pathname.startsWith('/api/admin/brackets'))response=await admin({request:req,env,data:{bracketImageFetch:async(url,init)=>{assert.equal(url,'https://images.example.com/contender.png');assert.equal(init.redirect,'manual');return new Response(await sharp({create:{width:96,height:64,channels:4,background:'#cfaa35'}}).png().toBuffer(),{headers:{'content-type':'image/png'}});}}});
  ...
  const scrolling=await board.locator('.bracket-viewport').evaluate(e=>({client:e.clientHeight,scroll:e.scrollHeight,max:getComputedStyle(e).maxHeight,bar:getComputedStyle(e,'::-webkit-scrollbar').height}));assert.ok(scrolling.scroll<=scrolling.client+1,JSON.stringify(scrolling));assert.equal(scrolling.max,'none');assert.equal(scrolling.bar,'3px');
  await page.evaluate(()=>window.scrollTo({top:800,behavior:'instant'}));const benchTop=await page.locator('.bracket-bench').evaluate(e=>e.getBoundingClientRect().top);assert.ok(benchTop>=78&&benchTop<100,`Sticky bench top ${benchTop}`);
  await board.locator('.bracket-viewport').hover({position:{x:150,y:100}});const beforeWheel=await page.evaluate(()=>scrollY);await page.mouse.wheel(0,250);await page.waitForTimeout(150);assert.ok(await page.evaluate(()=>scrollY)>beforeWheel);assert.equal(await board.locator('.bracket-viewport').evaluate(e=>e.scrollTop),0);await page.evaluate(()=>window.scrollTo({top:0,behavior:'instant'}));
  ...
  await page.getByRole('button',{name:'Edit round 1 match 1',exact:true}).click();assert.equal(await editor.getByLabel('Opponent 1 name',{exact:true}).isDisabled(),true);await editor.getByRole('button',{name:'Done',exact:true}).click();
  await page.getByRole('link',{name:'Library',exact:true}).click();await page.getByRole('button',{name:'Create season',exact:true}).click();await page.getByRole('dialog').getByLabel('Title',{exact:true}).fill('Direct text editing');await page.getByLabel('Starting slots',{exact:true}).selectOption('4');await page.getByRole('button',{name:'Create private draft',exact:true}).click();await page.getByRole('heading',{name:'Direct text editing',exact:true}).waitFor();
  await page.getByRole('button',{name:'Hide Ideas bench',exact:true}).click();await page.getByRole('button',{name:'Edit round 1 match 1',exact:true}).click();
  await editor.getByLabel('Opponent 1 name',{exact:true}).fill('Alpha');await editor.getByLabel('Opponent 2 name',{exact:true}).fill('Beta');await editor.getByLabel('Opponent 1 description',{exact:true}).fill('First contender description');await editor.getByLabel('Opponent 2 description',{exact:true}).fill('Second contender description');
  await page.screenshot({path:artifacts+'/direct-opponent-editing.png'});await editor.getByRole('button',{name:'Save draft and close',exact:true}).click();await editor.waitFor({state:'hidden'});await page.reload();await page.getByRole('button',{name:'Edit round 1 match 1',exact:true}).click();
  assert.equal(await editor.getByLabel('Opponent 1 name',{exact:true}).inputValue(),'Alpha');assert.equal(await editor.getByLabel('Opponent 2 name',{exact:true}).inputValue(),'Beta');assert.equal(await editor.getByLabel('Opponent 1 description',{exact:true}).inputValue(),'First contender description');assert.equal(await editor.getByLabel('Opponent 2 description',{exact:true}).inputValue(),'Second contender description');
  const originalId=await editor.getByLabel('Opponent 1 slot',{exact:true}).inputValue();await editor.getByLabel('Opponent 1 name',{exact:true}).fill('');await editor.getByRole('button',{name:'Save draft and close',exact:true}).click();await editor.getByRole('alert').filter({hasText:'Every contender needs a name'}).waitFor();
  await editor.getByLabel('Opponent 1 name',{exact:true}).fill('Alpha renamed');assert.equal(await editor.getByLabel('Opponent 1 slot',{exact:true}).inputValue(),originalId);await editor.getByRole('button',{name:'Save draft and close',exact:true}).click();await editor.waitFor({state:'hidden'});await page.reload();await page.getByRole('button',{name:'Edit round 1 match 1',exact:true}).click();assert.equal(await editor.getByLabel('Opponent 1 name',{exact:true}).inputValue(),'Alpha renamed');assert.equal(await editor.getByLabel('Opponent 1 slot',{exact:true}).inputValue(),originalId);assert.equal(await editor.getByLabel('Opponent 1 slot',{exact:true}).locator('option').count(),4);
  await page.setViewportSize({width:390,height:844});assert.equal(await editor.evaluate(e=>e.scrollWidth<=e.clientWidth),true);await editor.getByLabel('Opponent 2 name',{exact:true}).scrollIntoViewIfNeeded();await page.screenshot({path:artifacts+'/direct-opponent-editing-mobile.png'});await editor.getByRole('button',{name:'Done',exact:true}).click();
  await page.setViewportSize({width:1440,height:1000});await page.getByRole('button',{name:'Edit round 1 match 1',exact:true}).click();await editor.getByRole('button',{name:'Record manual result / bye',exact:true}).click();await page.getByLabel('Reason',{exact:true}).fill('Local partial-season regression');await page.getByRole('button',{name:'Confirm labelled decision & advance',exact:true}).click();await page.getByRole('dialog').waitFor({state:'hidden'});
  await page.getByRole('button',{name:'Expand Ideas bench',exact:true}).click();const expanded=page.getByRole('dialog',{name:'Ideas and contenders',exact:true});await expanded.waitFor();await expanded.getByLabel('Quick add / bulk paste',{exact:true}).fill('Gamma\nDelta');await expanded.getByRole('button',{name:'Add ideas',exact:true}).click();assert.deepEqual(await expanded.locator('.bracket-bench-item').evaluateAll(items=>items.map(i=>i.classList.contains('is-placed'))),[false,false,true,true]);assert.equal(await expanded.locator('.bracket-placed-check').count(),2);const gamma=expanded.locator('.bracket-bench-item').filter({hasText:'Gamma'});await gamma.locator('summary').click();await gamma.getByLabel('Name',{exact:true}).fill('Gamma from bench');await gamma.getByLabel('Gamma from bench image URL',{exact:true}).fill('https://images.example.com/contender.png');await gamma.getByRole('button',{name:'Use image URL',exact:true}).click();await gamma.locator('img').waitFor();await gamma.locator('summary').click();await page.screenshot({path:artifacts+'/expanded-ideas-bench.png'});await expanded.getByRole('button',{name:'Save draft and close',exact:true}).click();await expanded.waitFor({state:'hidden'});
  await page.getByRole('button',{name:'Edit round 1 match 2',exact:true}).click();assert.equal(await editor.getByLabel('Opponent 1 name',{exact:true}).isEnabled(),true);assert.equal(await editor.getByLabel('Opponent 2 slot',{exact:true}).isEnabled(),true);await editor.getByLabel('Opponent 1 name',{exact:true}).fill('Fresh contender');await editor.getByLabel('Opponent 2 slot',{exact:true}).selectOption({label:'Gamma from bench'});
  const png=await sharp({create:{width:96,height:64,channels:4,background:'#cfaa35'}}).png().toBuffer();for(const side of [1]){await editor.getByLabel(`Opponent ${side} feature image`,{exact:true}).setInputFiles({name:'contender.png',mimeType:'image/png',buffer:png});await page.waitForFunction(count=>document.querySelectorAll('dialog .bracket-opponent-artwork img').length===count,2);}
  await editor.getByLabel('Opponent 1 image URL',{exact:true}).fill('https://images.example.com/contender.png');await editor.locator('.bracket-image-url').first().getByRole('button',{name:'Use image URL',exact:true}).click();await page.waitForFunction(()=>document.querySelector('dialog input[aria-label="Opponent 1 image URL"]').value==='');
  await page.waitForFunction(()=>[...document.querySelectorAll('dialog .bracket-opponent-artwork img')].every(i=>i.complete&&i.naturalWidth>0));await page.screenshot({path:artifacts+'/partial-season-opponent-images.png'});await editor.getByRole('button',{name:'Save draft and close',exact:true}).click();await editor.waitFor({state:'hidden'});await page.reload();await page.getByRole('button',{name:'Edit round 1 match 2',exact:true}).click();assert.equal(await editor.getByLabel('Opponent 1 name',{exact:true}).inputValue(),'Fresh contender');assert.equal(await editor.getByLabel('Opponent 2 name',{exact:true}).inputValue(),'Gamma from bench');assert.equal(await editor.locator('.bracket-opponent-artwork img').count(),2);await editor.getByRole('button',{name:'Remove opponent 1 image',exact:true}).click();await editor.getByRole('button',{name:'Save draft and close',exact:true}).click();await editor.waitFor({state:'hidden'});await page.reload();await page.getByRole('button',{name:'Edit round 1 match 2',exact:true}).click();assert.equal(await editor.locator('.bracket-opponent-artwork img').count(),1);await editor.getByRole('button',{name:'Done',exact:true}).click();assert.equal(await board.locator('.bracket-winner-feature').count(),1);
