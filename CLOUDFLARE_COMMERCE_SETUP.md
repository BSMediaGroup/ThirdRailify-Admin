...
The catalogue action is `POST /api/admin/commerce/printful/catalogue/snapshot`, exposed as **Run read-only catalogue snapshot** on `/commerce/fulfillment`. One click uses signed phases on that same route so no Pages invocation approaches the 50 external-subrequest ceiling: bounded manifest pagination, product details, conditional file metadata, then provider-free verified assembly. Independently, all source and target GET starts share signed pacing metadata at a conservative 675 ms interval (about 88.9/minute) below Printful V1's published 120/minute quota. Every phase retains the authenticated Admin session, exact origin, CSRF, a bounded start or continuation rate budget, commerce D1, and Master or `commerce.integrations.manage`; only final assembly records completion. No provider credential is sent in any request body or response.

Treat Printful 419/429 as a recoverable provider pause. Preserve the signed exact cursor and partial normalized records, honor the later usable `Retry-After`/rate-reset time with a safety margin, and use the 62-second fallback when neither is usable. The open browser displays the absolute countdown and resumes automatically; calling before the deadline must make no Printful request. Transport/5xx retries are separately bounded. Do not enlarge a Pages chunk, restart from zero, disable Admin security, or introduce D1 catalogue-progress persistence to handle provider quotas.
