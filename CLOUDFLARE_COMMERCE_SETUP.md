...
This is a manual activation runbook for later, separately authorized milestones. None of these steps has been performed. Do not use it to activate live commerce.
...
- Stripe API identity has not been verified, no webhook is configured, and Checkout/live-payment gates are disabled.
...
Every mutation below requires separate approval at the milestone where it occurs.
...
5. Copy the dedicated Third Railify Stripe TEST secret key into the Admin encrypted secret `STRIPE_SECRET_KEY` without printing, committing, or inspecting its value in application tooling.
...
8. Implement and read-only verify the authenticated Stripe account identity; retrieve the account ID from Stripe rather than hardcoding it.
...
`STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` belong only in Admin Cloudflare encrypted secrets. Preserve the blank `STRIPE_PUBLISHABLE_KEY` scaffold as safe future configuration, but do not expose it to Public or require it for the initial Stripe-hosted Checkout URL redirect. A publishable key becomes relevant only if a separately approved embedded or client-side Stripe component needs Stripe.js.
