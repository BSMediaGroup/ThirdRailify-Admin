...
The transitional Pages callbacks listed above remain registered for rollback/API compatibility. Remove an old callback only after one legitimate custom-domain sign-in for that provider passes; never enable wildcard callback matching. Current activation and acceptance evidence is recorded in `docs/DOMAIN_CUTOVER.md`.
