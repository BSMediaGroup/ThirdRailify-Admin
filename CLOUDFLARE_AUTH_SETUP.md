...
Google production authentication uses the existing centralized Web application client and server callback, not Google Identity Services in the browser. Production has `GOOGLE_OAUTH_ENABLED=true` only after the operator confirmed the custom and transitional callback registrations, Branding/Audience state, and Search Console domain ownership. Preview deliberately retains `GOOGLE_OAUTH_ENABLED=false` until a separately reviewed preview-client decision. The production client requests only `openid email profile`; its secret remains the encrypted Admin-only `GOOGLE_CLIENT_SECRET`. Public and Admin both start against the same Account authority, while role and Master Admin resolution remains server-side.

The production callback generator uses only the four custom Admin URLs above. Google and X have legitimate custom-domain acceptance evidence, so their Pages callbacks are obsolete and await provider-dashboard removal. Discord and GitHub must retain their Pages callbacks until one legitimate custom-domain sign-in succeeds for each provider. Preview Google remains disabled and has no encrypted Google client secret. Remove only the provider-specific old callback after its acceptance boundary passes; never enable wildcard callback matching. Current activation and acceptance evidence is recorded in `docs/DOMAIN_CUTOVER.md`.
